...
import { createTeleCode } from "@telecode-ai/sdk"
...
`)
  ...
  const telecode = await createTeleCode({ port: 0 })
  ...
  try {
    ...
    const session = await telecode.client.session.create()
    const result = await telecode.client.session
      .prompt({
