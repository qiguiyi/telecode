...
Move the canonical TeleCode terminal application from
`packages/telecode/src/cli/cmd/tui` into a self-contained workspace package while
...
```text
packages/telecode ---\
                      > @telecode-ai/tui -> @telecode-ai/sdk
packages/cli --------/
```
...
generic presentation libraries. It must not depend on `packages/telecode`,
`packages/cli`, or `@telecode-ai/core`.

The SDK is the TUI's TeleCode boundary. Missing backend data or operations must
...
- Do not preserve private imports by creating aliases from `packages/tui` back
  into `packages/telecode`.
- Do not replace private `packages/telecode` imports with `@telecode-ai/core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@telecode-ai/tui`.

## Ownership Boundary

### `@telecode-ai/tui` Owns
...
- TeleCode domain data displayed by the TUI
...
```text
packages/telecode/src/cli/cmd/tui
```

Its private dependency on `packages/telecode` is primarily expressed through
the `@/*` TypeScript alias, which resolves to `packages/telecode/src/*`.
...
Status: Completed. The private `@telecode-ai/tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@telecode-ai/tui`.
...
- The package has no dependency on `telecode`, `@telecode-ai/cli`, or
  `@telecode-ai/core`.
...
primitives now live in `@telecode-ai/tui`. The legacy host consumes explicit
...
and build inputs through `@telecode-ai/tui/runtime`. Movable app, component,
route, and feature-plugin code no longer reads TeleCode globals or process state;
...
Replace process-global TeleCode state with resolved TUI inputs.
...
`@telecode-ai/tui/config` now owns schemas, defaults, keybind resolution, the
...
- Keep config path discovery, project/global precedence, migration, variable
  expansion, and plugin package installation in `packages/telecode` initially.
...
canonical in `@telecode-ai/tui`. Configured references resolve through the new
...
adapters rather than TeleCode domain implementations.

Make the SDK the only TeleCode domain boundary used by the TUI.
...
- Domain-facing TUI code imports TeleCode data and operations only from
  `@telecode-ai/sdk`.
...
`@telecode-ai/tui`. The legacy host injects a narrow plugin host that retains
...
- Move plugin rendering tests to `packages/tui`; retain installation/loading
  integration tests in `packages/telecode`.
...
- The package has no imports from `packages/telecode`, `packages/cli`, or
  `@telecode-ai/core`.
...
public `@telecode-ai/tui` root while retaining worker/server/config/plugin and
...
- Keep the legacy yargs commands corresponding to current `thread.ts` and
  `attach.ts` in `packages/telecode`.
- Keep the legacy embedded worker and server startup in `packages/telecode`.
- Change those adapters to load config, create transport inputs, and call the
  public `@telecode-ai/tui` API.
...
- Remove obsolete `@tui/*` path mappings from `packages/telecode`.
- Remove stale test fixtures and update all imports to package exports.
- Narrow `@telecode-ai/tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@telecode-ai/core`, or either
  executable package.
...
- TUI package imports do not reach into executable or backend implementation
  packages.
- SDK wire data is treated as the source of truth for TeleCode domain state.
...
cd packages/telecode && bun typecheck
...
rg '@telecode-ai/core|packages/telecode|packages/cli' packages/tui
rg 'src/cli/cmd/tui|@tui/' packages/telecode/src packages/telecode/test
```
...
- Build the current-platform `packages/telecode` binary.
