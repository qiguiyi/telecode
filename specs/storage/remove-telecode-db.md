# Remove `packages/telecode/src/storage/db.ts`

## Goal

Remove all production usages of the legacy `packages/telecode/src/storage/db.ts` module.
...
This does not mean removing SQLite or Drizzle everywhere in one step. The smaller target is deleting the telecode legacy wrapper by moving call sites onto deeper modules or onto the core/effect database adapter directly.

## Current Inventory

Production imports from `packages/telecode/src/storage/db.ts` are concentrated in 22 source files:

- `packages/telecode/src/account/repo.ts`
- `packages/telecode/src/cli/cmd/db.ts`
- `packages/telecode/src/cli/cmd/import.ts`
- `packages/telecode/src/cli/cmd/stats.ts`
- `packages/telecode/src/control-plane/workspace.ts`
- `packages/telecode/src/index.ts`
- `packages/telecode/src/node.ts`
- `packages/telecode/src/permission/index.ts`
- `packages/telecode/src/project/project.ts`
- `packages/telecode/src/server/projectors.ts`
- `packages/telecode/src/server/routes/instance/httpapi/handlers/sync.ts`
- `packages/telecode/src/server/shared/fence.ts`
- `packages/telecode/src/session/message-v2.ts`
- `packages/telecode/src/session/projectors.ts`
- `packages/telecode/src/session/prompt.ts`
- `packages/telecode/src/session/session.ts`
- `packages/telecode/src/session/todo.ts`
- `packages/telecode/src/share/share-next.ts`
- `packages/telecode/src/storage/db.ts`
- `packages/telecode/src/sync/index.ts`
- `packages/telecode/src/worktree/index.ts`
...
Status: Completed. Startup, the public node export, and database CLI tooling no longer import the legacy telecode database wrapper; `packages/telecode/src/storage/db.ts` has been deleted.
...
- Move database path and client startup behind the core/effect database module rather than the telecode wrapper.
...
Status: Completed. `SyncEvent` and the telecode projector boundary were removed; session/message event projection now lives in core EventV2/projector infrastructure.
...
Status: Completed. These services no longer import the legacy telecode database wrapper.
...
Status: Completed. Session/message reads and projector writes have moved off the legacy telecode database wrapper.
...
Status: Completed. Remaining one-off CLI/admin reads and writes now use core database services or domain services instead of the legacy telecode database wrapper.
...
All migration groups are complete or superseded. `packages/telecode/src/storage/db.ts` has been deleted.

## Superseded: Data Migrations

Status: Superseded. No telecode data-migration group remains.

The previous telecode `data-migration.ts` service only backfilled session usage from message rows. That work is now covered by core database migration `packages/core/src/database/migration/20260510033149_session_usage.ts`, so there is no separate telecode data-migration group.
...
- Existing schema ownership remains in `packages/core/src/**/*.sql.ts`; do not move table definitions back into `packages/telecode`.

## Verification Commands

- `rg "@/storage/db|./storage/db|Database\.(use|transaction|effect|Client|getPath)|\bTxOrDb\b|\bTransaction\b" packages/telecode/src`
- `bun typecheck` from `packages/telecode`
- Relevant package tests from `packages/telecode`, not the repo root
