...
Create a small workspace package that vendors the Drizzle `effect-sqlite` adapter shape for our repo. This is not an telecode storage abstraction. It is a local package that ports the Drizzle Effect SQLite implementation so we can use it before/independently of upstream release timing.

`packages/telecode` will use it internally, but the package itself should be generic: Drizzle + Effect + SQLite. No telecode paths, migrations, tables, transaction hooks, post-commit behavior, or domain language should live in this package.
...
- `@telecode-ai/effect-drizzle-sqlite`
...
- TeleCode-specific path/channel selection stays in `packages/telecode`.

## TeleCode Adoption Notes

These are not package requirements, but they matter for the later telecode adoption PR.

The current `packages/telecode/src/storage/db.ts` has two non-obvious semantics that the telecode wrapper must preserve when it consumes this adapter:
...
The telecode wrapper can implement that using Effect context instead of `LocalContext`:
...
Do not remove this behavior while moving telecode to Effect SQLite. `SyncEvent.run` depends on transaction composability and `behavior: "immediate"` for sequencing correctness.

## Migration Strategy

1. Add `@telecode-ai/effect-drizzle-sqlite` with a minimal in-memory/file SQLite test schema.
...
4. Add `@telecode-ai/effect-drizzle-sqlite` as a dependency of `packages/telecode`.
5. Port `packages/telecode/src/storage/db.ts` to be a thin compatibility wrapper over the adapter plus telecode-specific transaction/post-commit context.
...
8. Only then build domain stores like session/message/project stores on top of telecode's storage wrapper.
...
An Effect Drizzle SQLite package lets us vendor the adapter once. Then telecode can build its own storage wrapper on top, and `SessionStorage`, `MessageStorage`, event store, and projector writes can all share the same transaction and migration model.
...
- Should `afterCommit` stay telecode-specific until event publishing moves? Default answer: yes.
...
- Do CLI/admin raw SQL and sqlite shell stay in `packages/telecode`, or does the storage package expose backend capabilities for them?
...
- Use a tiny test schema, not telecode domain tables.
- Prove Effect Drizzle SQLite queries, transactions, and migrations.
- Do not migrate `packages/telecode` yet except possibly adding the dependency if needed for typechecking.

That gives us a focused place to validate the Effect SQLite approach before disturbing telecode's current database runtime.
