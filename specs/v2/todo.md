...
The telecode server has moved to the Effect HttpApi backend. Remaining work is
...
- remove the public in-memory `@telecode-ai/llm` tool loop after replacing its
...
- plugins get global "telecode" instance like in that post i showed
- telecode instance has stuff like `telecode.session.prompt()` or
  `telecode.tool.register({...})`
...
- decide whether to preserve deprecated `@telecode-ai/llm` orchestration exports
