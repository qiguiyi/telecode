...
Both `action` and `resource` use telecode's existing wildcard matching behavior.
...
The relative policy precedence of direct project files and `.telecode` files is intentionally deferred until `.telecode` configuration is reviewed.
...
Plugins must not be allowed to add, remove, or override policy statements. Plugins can contribute functionality or configured providers; policy determines whether telecode permits an operation through its managed execution paths.
