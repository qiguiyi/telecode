...
import { createTeleCode, type ToolPart } from "@telecode-ai/sdk"
...
console.log("🚀 Starting telecode server...")
const telecode = await createTeleCode({
  port: 0,
})
console.log("✅ TeleCode server ready")

const sessions = new Map<string, { client: any; server: any; sessionId: string; channel: string; thread: string }>()
void (async () => {
  const events = await telecode.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new telecode session...")
    const { client, server } = telecode
    ...
    console.log("✅ Created telecode session:", createResult.data.id)
  ...
  console.log("📝 Sending to telecode:", message.text)
  const result = await session.client.session.prompt({
    path: { id: session.sessionId },
    body: { parts: [{ type: "text", text: message.text }] },
  })

  console.log("📤 TeleCode response:", JSON.stringify(result, null, 2))
