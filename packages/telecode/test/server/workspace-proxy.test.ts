...
describe("HttpApi workspace proxy", () => {
  ...
  it.live("strips telecode-internal headers and merges extra headers", () =>
    Effect.gen(function* () {
      ...
      expect(forwarded["x-telecode-directory"]).toBeUndefined()
      expect(forwarded["x-telecode-workspace"]).toBeUndefined()
