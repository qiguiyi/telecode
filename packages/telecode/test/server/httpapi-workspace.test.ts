...
function requestServer(path: string, directory: string, init: RequestInit = {}) {
  ...
  headers.set("x-telecode-directory", directory)
      ...
      try {
        const response = yield* requestDefault(url.toString(), dir, {
          ...
          headers: {
            "accept-encoding": "br",
            "content-type": "application/json",
            "x-telecode-workspace": "internal",
          },
          body: JSON.stringify({ $schema: "https://telecode.ai/config.json" }),
        })
        ...
        expect(forwarded[0]?.headers).not.toHaveProperty("x-telecode-directory")
        expect(forwarded[0]?.headers).not.toHaveProperty("x-telecode-workspace")
