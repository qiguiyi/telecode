// Subprocess test harness for the telecode CLI. Spawns the real binary against
...
//
// Configuration flows through telecode's built-in test affordances:
//   - TELECODE_CONFIG_CONTENT      : provider config inline, no files to find
//   - TELECODE_TEST_HOME           : pins os.homedir() → tmpdir
//   - TELECODE_DISABLE_PROJECT_CONFIG : skip walking up for telecode.json
//   - TELECODE_PURE                : skip external plugin discovery + install
//   - TELECODE_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
// Plus HOME / XDG_* pointing at the tmpdir for belt-and-suspenders isolation.
//
// Today only `telecode.run` is fully wired. The shape supports adding more
// builders (`telecode.serve(opts)`, `telecode.acp(opts)`, `telecode.auth(...)`)
// without changing the fixture. Long-lived commands like `serve` will need a
// different return shape — see the TODO at the bottom of TeleCodeCli.
...
const telecodeRoot = path.resolve(import.meta.dir, "../../")
const cliEntry = path.join(telecodeRoot, "src/index.ts")
...
function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    TELECODE_TEST_HOME: home,
    HOME: home,
    XDG_CONFIG_HOME: path.join(home, ".config"),
    XDG_DATA_HOME: path.join(home, ".local/share"),
    XDG_STATE_HOME: path.join(home, ".local/state"),
    XDG_CACHE_HOME: path.join(home, ".cache"),
    TELECODE_CONFIG_CONTENT: configJson,
    TELECODE_DISABLE_PROJECT_CONFIG: "1",
    TELECODE_PURE: "1",
    TELECODE_DISABLE_AUTOUPDATE: "1",
    TELECODE_DISABLE_AUTOCOMPACT: "1",
    TELECODE_DISABLE_MODELS_FETCH: "1",
    TELECODE_AUTH_CONTENT: "{}",
  }
}
...
// Typed equivalent of constructing argv for `telecode run`. New flags should
...
// `telecode serve` is a long-lived process — it never exits on its own.
...
// `telecode acp` speaks newline-delimited JSON-RPC over stdin/stdout. It is
...
export type TeleCodeCli = {
  ...
  // Spawn `telecode serve` and wait until it's listening. Long-lived: the
  ...
  // Spawn `telecode acp` and return a duplex JSON-RPC handle. Long-lived:
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const spawn = Effect.fn("telecode.spawn")(function* (args: string[], opts?: SpawnOpts) {
    ...
    const runOpts = (opts?: RunOpts): SpawnOpts | undefined => {
      ...
      return {
        ...
        env: {
          ...opts.env,
          TELECODE_CONFIG_CONTENT: JSON.stringify({
    ...
    const startRun = Effect.fn("telecode.startRun")(function* (message: string, opts?: RunOpts) {
      const start = Date.now()
    ...
    const serve = Effect.fn("telecode.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
      ...
      //   "telecode server listening on http://<host>:<port>"
      ...
      const match = yield* Deferred.await(readyDeferred).pipe(
        Effect.timeoutOrElse({
          ...
          orElse: () =>
            Effect.fail(
              new Error(
                `telecode serve did not become ready within ${readyTimeoutMs}ms\n` +
    ...
    const acp = Effect.fn("telecode.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
    ...
    const telecode: TeleCodeCli = { run, startRun, serve, acp, spawn, expectExit, parseJsonEvents }

    return yield* fn({ llm, home, telecode })
    // FetchHttpClient is provided so test bodies can `yield* HttpClient.HttpClient`
    // and hit endpoints on `telecode.serve()` without rolling their own fetch.
  }).pipe(
    Effect.provide(
...
function expectExit(result: RunResult, expected: number, label = "telecode") {
...
// every call site. Use this for any test that needs the telecode CLI fixture.
//
...
// resources (e.g. `telecode.serve`) without an extra `Effect.scoped` wrapper —
