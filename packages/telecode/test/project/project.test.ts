...
import { ProjectDirectories } from "@telecode-ai/core/project/directories"
...
describe("Project.fromDirectory", () => {
  it.live("should handle git repository with no commits", () =>
    Effect.gen(function* () {
      ...
      const telecodeFile = path.join(tmp, ".git", "telecode")
      expect(yield* Effect.promise(() => Bun.file(telecodeFile).exists())).toBe(false)
    }),
  )
...
describe("Project.fromDirectory with worktrees", () => {
  ...
  it.live("worktree should share project ID with main repo", () =>
    Effect.gen(function* () {
      ...
      const cache = path.join(tmp, ".git", "telecode")
...
describe("Project.fromDirectory with bare repos", () => {
  it.live("worktree from bare repo should cache in bare repo, not parent", () =>
    Effect.gen(function* () {
      ...
      const correctCache = path.join(barePath, "telecode")
      const wrongCache = path.join(parentDir, ".git", "telecode")
  ...
  it.live("different bare repos under same parent should not share project ID", () =>
    Effect.gen(function* () {
      ...
      const cacheA = path.join(bareA, "telecode")
      const cacheB = path.join(bareB, "telecode")
      const wrongCache = path.join(parentDir, ".git", "telecode")
