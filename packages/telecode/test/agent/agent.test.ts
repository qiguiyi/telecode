...
it.instance("plan agent denies edits except .telecode/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".telecode/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".telecode", "skill", "perm-skill")
...
`,
      ...
      const home = process.env.TELECODE_TEST_HOME
      process.env.TELECODE_TEST_HOME = test.directory
