...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "telecode.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "telecode.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "telecode.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "telecode.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["telecode.json", "telecode.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "telecode.json"),
        path.join(project, "telecode.jsonc"),
      ])
    })
  })
