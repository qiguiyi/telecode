...
describe("tui thread", () => {
  ...
  cliIt.live("rejects mini-only options without --mini", ({ telecode }) =>
    Effect.gen(function* () {
      const result = yield* telecode.spawn(["--replay-limit", "10"])

      telecode.expectExit(result, 1)
      expect(result.stderr).toContain("--replay-limit requires --mini")
    }),
  )

  cliIt.live("routes attached sessions to mini mode", ({ telecode }) =>
    Effect.gen(function* () {
      const result = yield* telecode.spawn(["attach", "http://127.0.0.1:1", "--mini"])
  ...
  cliIt.live("rejects network options in mini mode", ({ telecode }) =>
    Effect.gen(function* () {
      const result = yield* telecode.spawn(["--mini", "--port", "4096"])
