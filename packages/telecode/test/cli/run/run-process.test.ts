// Subprocess integration tests for `telecode run` (non-interactive mode).
// These exercise the real CLI binary against a TestLLMServer running in the
// same process. See `test/lib/cli-process.ts` for the harness — each test uses
// `telecode.run(message, opts?)` to spawn `bun src/index.ts run ...` with
// `TELECODE_CONFIG_CONTENT` providing the test provider config inline.
import { describe, expect } from "bun:test"
import { Effect } from "effect"
import { reply } from "../../lib/llm-server"
import { cliIt } from "../../lib/cli-process"

describe("telecode run (non-interactive subprocess)", () => {
  ...
  cliIt.concurrent(
    ...
    ({ llm, telecode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* telecode.run("say hi")
  ...
  cliIt.concurrent(
    ...
    ({ llm, telecode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* telecode.run("use a tool", {
  ...
  cliIt.concurrent(
    ...
    ({ llm, telecode }) =>
      Effect.gen(function* () {
        ...
        const thinking = yield* telecode.run("think", { extraArgs: ["--thinking"] })
        telecode.expectExit(thinking, 0)
        expect(thinking.stdout).toBe("Thinking: considering\nanswer\n")

        yield* llm.reason("hidden", { text: "visible" })
        const plain = yield* telecode.run("think again")
        telecode.expectExit(plain, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, telecode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* telecode.run("trigger midstream error", { timeoutMs: 30_000 })
  ...
  cliIt.concurrent(
    ...
    ({ llm, telecode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* telecode.run("say hi", { format: "json" })
        telecode.expectExit(result, 0)

        const events = telecode.parseJsonEvents(result.stdout)
  ...
  cliIt.concurrent(
    ...
    ({ telecode }) =>
      Effect.gen(function* () {
        const result = yield* telecode.run("use an unknown model", {
          model: "test/nonexistent-model",
          format: "json",
        })
  ...
  cliIt.concurrent(
    ...
    ({ llm, telecode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* telecode.run("exercise json records", {
          format: "json",
  ...
  cliIt.concurrent(
    ...
    ({ llm, telecode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* telecode.run("fail after output", { format: "json" })
  ...
  cliIt.concurrent(
    ...
    ({ home, llm, telecode }) =>
      Effect.gen(function* () {
        ...
        const denied = yield* telecode.run("request permission", { permission: { bash: "ask" } })
        telecode.expectExit(denied, 0)
        ...
        const allowed = yield* telecode.run("request permission", {
          permission: { bash: "ask" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        telecode.expectExit(allowed, 0)
        ...
        const explicitlyDenied = yield* telecode.run("request denied permission", {
          permission: { bash: "deny" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        telecode.expectExit(explicitlyDenied, 0)
  ...
  cliIt.live(
    ...
    ({ home, llm, telecode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* telecode.run("read the attachment", {
  ...
  cliIt.concurrent(
    ...
    ({ home, telecode }) =>
      Effect.gen(function* () {
        const result = yield* telecode.run("read the directory", {
  ...
  cliIt.live(
    ...
    ({ llm, telecode }) =>
      Effect.gen(function* () {
        yield* llm.hang
        const run = yield* telecode.startRun("wait forever")
        yield* llm.wait(1)
        run.interrupt()
