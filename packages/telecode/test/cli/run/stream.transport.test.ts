...
import { TeleCodeClient, type GlobalEvent } from "@telecode-ai/sdk/v2"
import { createSessionTransport } from "@/cli/cmd/run/stream.transport"
import type { FooterApi, FooterEvent, LocalReplayRow, RunFilePart, StreamCommit } from "@/cli/cmd/run/types"

type EventStream = Awaited<ReturnType<TeleCodeClient["event"]["subscribe"]>>["stream"]
type GlobalEventStream = Awaited<ReturnType<TeleCodeClient["global"]["event"]>>["stream"]
type SdkEvent = EventStream extends AsyncGenerator<infer T, unknown, unknown> ? T : never
type SessionMessage = NonNullable<Awaited<ReturnType<TeleCodeClient["session"]["messages"]>>["data"]>[number]
type SessionChild = NonNullable<Awaited<ReturnType<TeleCodeClient["session"]["children"]>>["data"]>[number]
type SessionToolPart = Extract<SessionMessage["parts"][number], { type: "tool" }>
type SessionStatusMap = NonNullable<Awaited<ReturnType<TeleCodeClient["session"]["status"]>>["data"]>
...
function ok<T>(data: T) {
  return Promise.resolve({
    data,
    error: undefined,
    request: new Request("https://telecode.test"),
...
function sdk(
  input: {
    ...
    subscribe?: TeleCodeClient["event"]["subscribe"]
    globalEvent?: TeleCodeClient["global"]["event"]
    promptAsync?: TeleCodeClient["session"]["promptAsync"]
    status?: TeleCodeClient["session"]["status"]
    messages?: TeleCodeClient["session"]["messages"]
    children?: TeleCodeClient["session"]["children"]
    permissions?: TeleCodeClient["permission"]["list"]
    questions?: TeleCodeClient["question"]["list"]
  } = {},
) {
  const client = new TeleCodeClient()

  const subscribe: TeleCodeClient["event"]["subscribe"] = input.subscribe ?? (() => sse(input.stream ?? emptyStream()))
  const globalEvent: TeleCodeClient["global"]["event"] =
    input.globalEvent ?? (() => globalSse(input.globalStream ?? wrapGlobalStream(input.stream ?? emptyStream())))
  const promptAsync: TeleCodeClient["session"]["promptAsync"] = input.promptAsync ?? (() => ok(undefined))
  const status: TeleCodeClient["session"]["status"] = input.status ?? (() => ok({}))
  const messages: TeleCodeClient["session"]["messages"] = input.messages ?? (() => ok([]))
  const children: TeleCodeClient["session"]["children"] = input.children ?? (() => ok([]))
  const permissions: TeleCodeClient["permission"]["list"] = input.permissions ?? (() => ok([]))
  const questions: TeleCodeClient["question"]["list"] = input.questions ?? (() => ok([]))
