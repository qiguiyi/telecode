...
describe("telecode read-only commands (smoke)", () => {
  // `mcp list` reads MCP server config and pings each one. With the empty
  // TELECODE_CONFIG_CONTENT={} we provide, no servers should be configured
  // and the command should report that cleanly.
  cliIt.live(
    "mcp list: exits 0",
    ({ telecode }) =>
      Effect.gen(function* () {
        const r = yield* telecode.spawn(["mcp", "list"])
        telecode.expectExit(r, 0, "mcp list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ telecode }) =>
      Effect.gen(function* () {
        const r = yield* telecode.spawn(["providers", "list"])
        telecode.expectExit(r, 0, "providers list")
  ...
  cliIt.live(
    ...
    ({ telecode }) =>
      Effect.gen(function* () {
        const r = yield* telecode.spawn(["models"])
        telecode.expectExit(r, 0, "models")
  ...
  cliIt.live(
    ...
    ({ telecode }) =>
      Effect.gen(function* () {
        const r = yield* telecode.spawn(["agent", "list"])
        telecode.expectExit(r, 0, "agent list")
      }),
    60_000,
  )

  // `session list` reads the session DB. Fresh TELECODE_TEST_HOME means
  // empty DB. Exit 0 with no sessions.
  cliIt.live(
    "session list: exits 0",
    ({ telecode }) =>
      Effect.gen(function* () {
        const r = yield* telecode.spawn(["session", "list"])
        telecode.expectExit(r, 0, "session list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ telecode }) =>
      Effect.gen(function* () {
        const r = yield* telecode.spawn(["stats"])
        telecode.expectExit(r, 0, "stats")
      }),
    60_000,
  )
  ...
  // in production it'd be a path under TELECODE_TEST_HOME / XDG_DATA_HOME.
  // Accept either form — both prove the resolver ran without crashing.
  cliIt.live(
    "db path: exits 0 and prints a path or :memory:",
    ({ telecode }) =>
      Effect.gen(function* () {
        const r = yield* telecode.spawn(["db", "path"])
        telecode.expectExit(r, 0, "db path")
