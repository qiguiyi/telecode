...
// terminal sizes. The default telecode tui command is excluded —
// `telecode --help` includes an ASCII banner that pulls in the install
...
// Top-level commands. Order matches what `telecode --help` prints today;
...
// top-level help on `--help` and exits 1; not a real telecode command.
const TOP_LEVEL = [
  "acp",
  "mcp",
  "attach",
...
describe("telecode CLI help-text snapshots", () => {
  ...
  cliIt.live(
    ...
    ({ telecode }) =>
      Effect.gen(function* () {
        const topLevel = yield* telecode.spawn(["--help"], { env: SNAPSHOT_ENV })
        ...
        const [failures, results] = yield* Effect.partition(
          ...
          (argv) =>
            Effect.gen(function* () {
              const result = yield* telecode.spawn([...argv, "--help"], { env: SNAPSHOT_ENV })
              if (result.exitCode !== 0) {
                return yield* Effect.fail(`telecode ${argv.join(" ")}: exit ${result.exitCode}`)
              }
              return { argv, result }
            }),
          { concurrency: 8 },
        ...
        for (const { argv, result } of results) {
          ...
          expect(normalize(result.stderr)).toMatchSnapshot(`telecode ${argv.join(" ")} --help`)
        }
