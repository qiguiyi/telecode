...
describe("telecode acp initialize/auth subprocess", () => {
  cliIt.live(
    "initialize responds with capabilities",
    ({ telecode }) =>
      Effect.gen(function* () {
        const initialized = yield* initialize(yield* createAcpClient({ telecode }))
        ...
        expect(initialized.agentInfo?.name).toBe("TeleCode")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ telecode }) =>
      Effect.gen(function* () {
        const acp = yield* createAcpClient({ telecode })
        const initialized = yield* initialize(acp)

        expect(initialized.authMethods?.[0]?.id).toBe("telecode-login")
        expect(initialized.authMethods?.[0]?._meta?.["terminal-auth"]).toBeDefined()
        expect(yield* acp.request<AuthenticateResponse>("authenticate", { methodId: "telecode-login" })).toMatchObject({
          result: {},
        })
        ...
        expect(JSON.stringify(rejected.error)).not.toContain(process.env.TELECODE_AUTH_CONTENT ?? "not-present")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ telecode }) =>
      Effect.gen(function* () {
        ...
        expect(initialized.result?.authMethods?.[0]?.id).toBe("telecode-login")
