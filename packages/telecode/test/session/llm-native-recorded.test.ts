...
import { SessionV1 } from "@telecode-ai/core/v1/session"
import { FSUtil } from "@telecode-ai/core/fs-util"
import { ModelsDev } from "@telecode-ai/core/models-dev"
import { HttpRecorder } from "@telecode-ai/http-recorder"
import { HttpRecorderInternal } from "@telecode-ai/http-recorder/internal"
...
import { LLMEvent, LLMResponse } from "@telecode-ai/llm"
import { LLMClient, RequestExecutor, WebSocketExecutor } from "@telecode-ai/llm/route"
import { Env } from "@/env"
...
const zenURL = (connection: string) => `https://console.telecode.ai/proxy/connections/${connection}/v1`
...
function decodeRecordOpenAIOAuth() {
  const value = process.env.TELECODE_RECORD_OPENAI_AUTH
...
const RECORDED_SCENARIOS = [
  {
    ...
    tags: ["telecode", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("TELECODE_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.openai,
        name: "OpenAI",
        ...
        model,
        options: {
          apiKey: envValue("TELECODE_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY") ?? "fixture-openai-key",
  ...
  },
  {
    ...
    protocol: "openai-responses",
    tags: ["telecode", "native", "oauth", "tool-loop"],
  ...
  },
  {
    id: "telecode-proxy",
    name: "TeleCode proxy",
    providerID: ProviderV2.ID.telecode,
    modelID: "gpt-5.2-codex",
    cassette: "session/native-zen-tool-loop",
    protocol: "openai-responses",
    tags: ["telecode", "zen", "native", "tool-loop"],
    canRecord: () => Boolean(process.env.TELECODE_RECORD_CONSOLE_TOKEN && process.env.TELECODE_RECORD_ZEN_ORG_ID),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.telecode,
        name: "TeleCode Zen",
        env: ["TELECODE_CONSOLE_TOKEN"],
        npm: "@ai-sdk/openai-compatible",
        api: zenURL(process.env.TELECODE_RECORD_ZEN_CONNECTION ?? "fixture"),
        model,
        options: {
          apiKey: process.env.TELECODE_RECORD_CONSOLE_TOKEN ?? "fixture-console-token",
          headers: { "x-org-id": process.env.TELECODE_RECORD_ZEN_ORG_ID ?? "fixture-org" },
        },
      }),
  },
  {
    ...
    tags: ["telecode", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("TELECODE_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.anthropic,
        name: "Anthropic",
        ...
        model,
        options: {
          apiKey: envValue("TELECODE_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY") ?? "fixture-anthropic-key",
...
const selectedScenarios = new Set(
  (envValue("TELECODE_RECORDED_SCENARIO", "RECORDED_PROVIDER") ?? "")
    .split(",")
...
const recordError = (scenario: RecordedScenario) =>
  scenario.id === "openai-oauth"
    ? "Set TELECODE_RECORD_OPENAI_AUTH to an OAuth auth JSON object in the recording environment."
...
function recordedNativeLLMLayer(scenario: RecordedScenario) {
  ...
  // Only the HTTP client is recorded; RequestExecutor and the telecode LLM stack remain real.
  const metadata = {
...
const writeConfig = (directory: string, scenario: RecordedScenario, model: ModelsDev.Provider["models"][string]) =>
  Effect.promise(() =>
    Bun.write(
      path.join(directory, "telecode.json"),
      JSON.stringify({ $schema: "https://telecode.ai/config.json", ...scenario.config(model) }),
    ),
  )
