...
Trim `packages/telecode/src/server/routes/instance/httpapi/public.ts` until OpenAPI generation is mostly a direct projection of the `HttpApi` route declarations, without breaking the generated SDK surface.
...
- `[x]` Add or extend `packages/telecode/test/server/httpapi-query-schema-drift.test.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/telecode`.
- `[x]` `bun typecheck` from `packages/telecode`.

### PR 2: Delete Spec-Only Workspace Query Injection

- `[x]` Edit `packages/telecode/src/server/routes/instance/httpapi/public.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/telecode`.
- `[x]` `bun dev generate > /tmp/telecode-openapi.json` from `packages/telecode`.
...
- Edit `packages/telecode/src/server/routes/instance/httpapi/public.ts`.
...
- `bun dev generate > /tmp/telecode-openapi.json` from `packages/telecode`.
...
- Check source schemas in files like `packages/telecode/src/session/schema.ts`, `packages/telecode/src/permission/schema.ts`, and pty schema definitions.
...
- Edit route group files under `packages/telecode/src/server/routes/instance/httpapi/groups/`.
- Replace SDK-visible `HttpApiError.BadRequest` / `HttpApiError.NotFound` with explicit error schemas from `packages/telecode/src/server/routes/instance/httpapi/errors.ts` or add new ones there.
