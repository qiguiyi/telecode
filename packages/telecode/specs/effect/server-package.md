...
Practical reference for a future `packages/server` split after the telecode
server moved to the Effect HttpApi backend.

## Current State

- The server still lives in `packages/telecode`.
...
Until enough shared service code lives outside `packages/telecode`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/telecode`

It should not import `packages/telecode` services while `packages/telecode`
...
2. Move stable domain schemas into shared packages only when they no longer
   depend on telecode-local runtime modules.
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/telecode` implementation details.
