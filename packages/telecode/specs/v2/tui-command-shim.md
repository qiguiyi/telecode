...
Delete `packages/telecode/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/telecode/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/telecode/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/telecode`
- TUI plugin loader tests in `packages/telecode` if runtime plugin API wiring changed
