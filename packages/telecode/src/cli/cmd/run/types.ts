// Shared type vocabulary for the direct interactive mode (`telecode --mini`).
//
...
import type { TeleCodeClient, PermissionRequest, QuestionRequest, ToolPart } from "@telecode-ai/sdk/v2"
import type { TuiConfig } from "@telecode-ai/tui/config"
...
type PromptModel = Parameters<TeleCodeClient["session"]["prompt"]>[0]["model"]
type PromptInput = Parameters<TeleCodeClient["session"]["prompt"]>[0]

export type RunPromptPart = NonNullable<PromptInput["parts"]>[number]

export type RunCommand = NonNullable<Awaited<ReturnType<TeleCodeClient["command"]["list"]>>["data"]>[number]

export type RunProvider = NonNullable<Awaited<ReturnType<TeleCodeClient["provider"]["list"]>>["data"]>["all"][number]
...
export type RunAgent = NonNullable<Awaited<ReturnType<TeleCodeClient["app"]["agents"]>>["data"]>[number]

type RunResourceMap = NonNullable<Awaited<ReturnType<TeleCodeClient["experimental"]["resource"]["list"]>>["data"]>
...
export type PermissionReply = Parameters<TeleCodeClient["permission"]["reply"]>[0]

export type QuestionReply = Parameters<TeleCodeClient["question"]["reply"]>[0]

export type QuestionReject = Parameters<TeleCodeClient["question"]["reject"]>[0]
