import type { PermissionV1 } from "@telecode-ai/core/v1/permission"
import { FSUtil } from "@telecode-ai/core/fs-util"
// CLI entry point for `telecode run` and `telecode --mini`.
//
// Handles three modes:
//   1. Non-interactive (default): sends a single prompt, streams events to
//      stdout, and exits when the session goes idle.
//   2. Interactive local (`telecode --mini`): boots the split-footer direct mode
//      with an in-process server (no external HTTP).
//   3. Interactive attach (`telecode --mini --attach`): connects to a running
//      telecode server and runs interactive mode against it.
//
...
import { createTeleCodeClient, type TeleCodeClient, type ToolPart } from "@telecode-ai/sdk/v2"
import { FormatError, FormatUnknownError } from "../error"
import { INTERACTIVE_INPUT_ERROR, resolveInteractiveStdin } from "./run/runtime.stdin"

type ModelInput = Parameters<TeleCodeClient["session"]["prompt"]>[0]["model"]
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run telecode with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running telecode server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to TELECODE_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to TELECODE_SERVER_USERNAME or 'telecode')",
      })
      .option("dir", {
        type: "string",
  ...
  handler: Effect.fn("Cli.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      async function session(sdk: TeleCodeClient): Promise<SessionInfo | undefined> {
        if (args.session) {
      ...
      async function share(sdk: TeleCodeClient, sessionID: string) {
      ...
      async function current(sdk: TeleCodeClient): Promise<string> {
        if (!args.attach) {
      ...
      async function attachAgent(sdk: TeleCodeClient) {
        if (!args.agent) return undefined
        const name = args.agent
      ...
      async function pickAgent(sdk: TeleCodeClient) {
        if (!args.agent) return undefined
        if (args.attach) {
      ...
      async function execute(sdk: TeleCodeClient) {
        ...
        async function loop(client: TeleCodeClient, events: Awaited<ReturnType<typeof sdk.event.subscribe>>) {
      ...
      const sdk = createTeleCodeClient({
        baseUrl: "http://telecode.internal",
        fetch: fetchFn,
        directory,
      })
      await execute(sdk)
