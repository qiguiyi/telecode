...
import type { Message, TeleCodeClient, SessionMessageResponse } from "@telecode-ai/sdk/v2"
...
export const AuthMethodID = "telecode-login"
...
export class Service extends Context.Service<Service, Interface>()("@telecode/ACP/Service") {}
...
export function make(input: {
  ...
  const initialize = Effect.fn("ACP.initialize")(function* (params: InitializeRequest) {
    ...
    const authMethod: AuthMethod = {
      description: "Run `telecode auth login` in the terminal",
      name: "Login with telecode",
      id: AuthMethodID,
    }
...
function makeDirectoryService(sdk: TeleCodeClient) {
...
function makeUsageService(sdk: TeleCodeClient) {
...
async function loadDirectorySnapshot(sdk: TeleCodeClient, directory: string) {
...
function defaultModelFromConfig(
  ...
  // a default. Configured model, telecode provider, then sorted best model keep
  // the protocol response deterministic without extra session/message reads.
  const telecodeProvider = providers[ProviderV2.ID.make("telecode")]
  const telecodeModel = telecodeProvider ? Provider.sort(Object.values(telecodeProvider.models))[0] : undefined
  if (telecodeProvider && telecodeModel) return { providerID: telecodeProvider.id, modelID: telecodeModel.id }
...
function fromUnknownError(error: unknown, service?: string): Error {
  ...
  return new ACPError.ServiceFailureError({ safeMessage: "TeleCode service failure", service })
}
