...
import {
  ...
  type TuiSlotPlugin,
  type TuiTheme,
} from "@telecode-ai/plugin/tui"
import path from "path"
import { fileURLToPath } from "url"
import { TuiConfig } from "@/config/tui"
import { errorData, errorMessage } from "@telecode-ai/tui/util/error"
...
import { hasTheme, upsertTheme } from "@telecode-ai/tui/context/theme"
import { Global } from "@telecode-ai/core/global"
import { Filesystem } from "@/util/filesystem"
import { Process } from "@/util/process"
import { Flock } from "@telecode-ai/core/util/flock"
import { Flag } from "@telecode-ai/core/flag/flag"
import { internalTuiPlugins, type InternalTuiPlugin } from "./internal"
import type { HostPluginApi, HostSlots } from "@telecode-ai/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@telecode-ai/core/v1/config/plugin"
import { createCommandShim } from "@telecode-ai/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@telecode-ai/tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".telecode"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".telecode", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".telecode", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.TELECODE_PURE ? [] : pluginOrigins
    if (Flag.TELECODE_PURE && pluginOrigins.length) {
    }
