...
export const DEPRECATED_PLUGIN_PACKAGES = ["telecode-openai-codex-auth", "telecode-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, telecodeVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(telecodeVersion) || semver.major(telecodeVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.telecode
  if (typeof range !== "string") return
  if (!semver.satisfies(telecodeVersion, range)) {
    throw new Error(`Plugin requires telecode ${range} but running ${telecodeVersion}`)
  }
}
