...
const TUI_SCHEMA_URL = "https://telecode.ai/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from telecode.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const telecode = await telecodeFiles(input)
  for (const file of telecode) {
...
async function telecodeFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "telecode"),
    ...(await Filesystem.findUp(["telecode.json", "telecode.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "telecode"))
  }
  if (Flag.TELECODE_CONFIG) files.push(Flag.TELECODE_CONFIG)
