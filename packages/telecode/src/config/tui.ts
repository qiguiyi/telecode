...
import { isRecord } from "@telecode-ai/tui/util/record"
...
import { TuiKeybind } from "@telecode-ai/tui/config/keybind"
...
import { TuiConfig } from "@telecode-ai/tui/config"
...
export class Service extends Context.Service<Service, Interface>()("@telecode/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old telecode.json shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, any `.telecode`
  // folders between cwd and home, and TELECODE_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.TELECODE_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit TELECODE_TUI_CONFIG override, if set.
  if (Flag.TELECODE_TUI_CONFIG) {
    const configFile = Flag.TELECODE_TUI_CONFIG
  ...
  // 4. `.telecode` directories (and TELECODE_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".telecode") || dir === Flag.TELECODE_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".telecode") && dir !== Flag.TELECODE_CONFIG_DIR) continue
