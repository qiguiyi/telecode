...
import type { LLMClientShape } from "@telecode-ai/llm/route"
...
function statusWithFetch(
  ...
  if (providerID !== "openai" && providerID !== "anthropic" && !providerID.startsWith("telecode"))
    return { type: "unsupported", reason: "provider is not openai, telecode, or anthropic" }
...
export function stream(input: StreamInput): StreamResult {
  ...
  // Integration point with @telecode-ai/llm: native-request lowers session data
...
export function nativeTools(tools: Record<string, Tool>, input: Pick<StreamInput, "messages" | "abort">) {
  return Object.fromEntries(
    Object.entries(tools).map(([name, item]) => [
      name,
      // Tool execution remains telecode-owned. The native runtime only adapts
      // the @telecode-ai/llm tool call back into the AI SDK Tool.execute shape.
      NativeTool.make({
