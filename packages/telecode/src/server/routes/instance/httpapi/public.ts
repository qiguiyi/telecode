...
import { TeleCodeHttpApi } from "./api"
...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for telecode serve and web commands",
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw: OpenApiSpec = OpenApi.fromApi(TeleCodeHttpApi)
...
export const PublicApi = TeleCodeHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "telecode",
    version: "1.0.0",
    description: "telecode api",
