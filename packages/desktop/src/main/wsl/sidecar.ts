...
import { type WslCommandLine, resolveWslTeleCode, shellEscape, wslArgs } from "./runtime"
...
export async function spawnWslSidecar(
  ...
  const telecode = await resolveWslTeleCode(distro)
  if (!telecode) throw new Error(`TeleCode is not installed in ${distro}`)

  const port = await allocatePort()
  const password = randomUUID()
  const username = "telecode"
  const script = [
    "set -euo pipefail",
    'cd "$HOME" || cd /',
    ...
    "export PATH",
    "export WSLENV=",
    "export TELECODE_EXPERIMENTAL_DISABLE_FILEWATCHER=true",
    "export TELECODE_CLIENT=desktop",
    `export TELECODE_SERVER_USERNAME=${shellEscape(username)}`,
    `export TELECODE_SERVER_PASSWORD=${shellEscape(password)}`,
    'export XDG_STATE_HOME="$HOME/.local/state"',
    `exec ${shellEscape(telecode)} --print-logs --log-level ${app.isPackaged ? "WARN" : "INFO"} serve --hostname 0.0.0.0 --port ${port}`,
  ].join("\n")
