...
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.telecode.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `telecode.settings.dat` is special: it maps to the `telecode.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"telecode.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // telecode.settings.dat → the electron settings store ("telecode.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "telecode.global.dat").
  const storeName = filename === "telecode.settings.dat" ? "telecode.settings" : filename
