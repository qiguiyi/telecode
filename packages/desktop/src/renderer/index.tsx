...
import {
  ...
  useCommand,
  useWslServers,
} from "@telecode-ai/app"
import type { UpdaterState } from "@telecode-ai/app/updater"
    ...
    integrations: (integrations) => {
      return integrations.filter(
        (i) =>
          i.name !== "Breadcrumbs" &&
          !(
            import.meta.env.TELECODE_CHANNEL === "prod" &&
...
const deepLinkEvent = "telecode:deep-link"

const emitDeepLinks = (urls: string[]) => {
  if (urls.length === 0) return
  window.__TELECODE__ ??= {}
  const pending = window.__TELECODE__.deepLinks ?? []
  window.__TELECODE__.deepLinks = [...pending, ...urls]
  ...
  return {
    ...
    notify: async (title, description, href) => {
      ...
      const notification = new Notification(title, {
        body: description ?? "",
        icon: "https://telecode.ai/favicon-96x96-v3.png",
      })
...
render(() => {
  ...
  const loadLocale = async () => {
    const current = await platform.storage?.("telecode.global.dat").getItem("language")
