...
function clearHighlightFind() {
  ...
  if (!api) return
  api.delete("telecode-find")
  api.delete("telecode-find-current")
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    ...
    if (active) api.set("telecode-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== currentIndex)
    if (rest.length > 0) api.set("telecode-find", new Highlight(...rest))
    return true
  }
