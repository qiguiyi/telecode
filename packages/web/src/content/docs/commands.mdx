...
Create `.telecode/commands/test.md`:
...
You can add custom commands through the TeleCode config or by creating markdown files in the `commands/` directory.

---

### JSON

Use the `command` option in your TeleCode [config](/docs/config):
...
telecode includes several built-in commands like `/init`, `/undo`, `/redo`, `/share`, `/help`; [learn more](/docs/tui#commands).
