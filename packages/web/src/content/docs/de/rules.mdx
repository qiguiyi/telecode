---
title: Regeln
description: Lege eigene Anweisungen fuer telecode fest.
---

Du kannst telecode ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in telecode den Befehl `/init` aus.
...
So kann telecode sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht telecode sie zu erweitern.
...
telecode kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/telecode/AGENTS.md` speichern.
Diese gelten in allen telecode-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt TeleCode dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/telecode/AGENTS.md` existiert)
...
Beim Start sucht telecode in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/telecode/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `telecode.json` oder in `~/.config/telecode/telecode.json` angeben.
...
telecode parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit telecode.json

Empfohlen ist das `instructions`-Feld in der `telecode.json`:
...
Du kannst telecode explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `telecode.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
