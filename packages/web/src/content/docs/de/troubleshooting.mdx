...
Wenn TeleCode Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\telecode\log`

Dateinamen enthalten Zeitstempel (z. B. `2025-01-09T123456.log`) und es bleiben die letzten 10 Logs erhalten.

Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `telecode --log-level DEBUG`.

---

## Speicher

telecode speichert Sitzungs- und App-Daten auf der Festplatte unter:

- **macOS/Linux**: `~/.local/share/telecode/`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\telecode`
...
TeleCode Desktop startet im Hintergrund einen lokalen TeleCode-Server (`telecode-cli`-Sidecar).
...
- Nur macOS: `TeleCode`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
- **macOS/Linux**: `~/.config/telecode/telecode.jsonc` (oder `~/.config/telecode/telecode.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/telecode/telecode.jsonc`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\telecode\telecode.jsonc`
...
TeleCode kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:

- **Globale Plugins**
  - **macOS/Linux**: `~/.config/telecode/plugins/`
  - **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\telecode\plugins`
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit TeleCode ihn neu aufbauen kann.

1. Beende TeleCode Desktop komplett.
2. Loesche das Cache-Verzeichnis:

- **macOS**: Finder -> `Cmd+Shift+G` -> einfuegen: `~/.cache/telecode`
- **Linux**: loesche `~/.cache/telecode` (oder fuehre aus: `rm -rf ~/.cache/telecode`)
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.cache\telecode`

3. Starte TeleCode Desktop neu.

---

### Server-Verbindungsprobleme beheben

TeleCode Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Wenn deine `telecode.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.

#### Umgebungsvariablen prüfen

Wenn du `TELECODE_PORT` in deiner Umgebung gesetzt hast, versucht die Desktop-App diesen Port fuer den lokalen Server zu nutzen.

- Setze `TELECODE_PORT` zurueck (oder waehle einen freien Port) und starte neu.
...
Unter Windows benoetigt TeleCode Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit TeleCode-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

TeleCode Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer TeleCode in den OS-Einstellungen aktiviert sind, und
...
1. Beende TeleCode Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von TeleCode Desktop):

- `telecode.settings.dat` (desktop default server URL)
- `telecode.global.dat` und `telecode.workspace.*.dat` (UI state like recent servers/projects)
...
Wenn du Probleme mit TeleCode hast:
...
Um zu sehen, auf welche Modelle du Zugriff hast, fuehre `telecode models` aus.
...
2. Wenn das Problem besteht, versuche deine gespeicherte Konfiguration zu loeschen:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.local\share\telecode`
...
Wenn du API-Call-Fehler erhaeltst, kann das an veralteten Provider-Paketen liegen. telecode installiert Provider-Pakete (OpenAI, Anthropic, Google, etc.) dynamisch bei Bedarf und cached sie lokal.
...
1. Leere den Provider-Paket-Cache:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.cache\telecode`

2. Starte telecode neu, um die neuesten Provider-Pakete zu installieren

Dies zwingt telecode, die neuesten Versionen der Provider-Pakete herunterzuladen, was oft Kompatibilitaetsprobleme mit Modellparametern und API-Aenderungen loest.
...
telecode erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).
