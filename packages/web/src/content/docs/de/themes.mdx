...
Mit TeleCode kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet TeleCode das `telecode`-Theme.
...
TeleCode liefert mehrere eingebaute Themes mit.
...
- TeleCode optisch ans Terminal anpassen wollen
...
TeleCode unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/telecode/themes/*.json` oder `$XDG_CONFIG_HOME/telecode/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.telecode/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.telecode/themes/*.json`
