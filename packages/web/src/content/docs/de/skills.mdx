...
Agent Skills erlauben TeleCode, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
TeleCode sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft TeleCode vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.telecode/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/telecode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.telecode/skills/git-release/SKILL.md` zum Beispiel so:
...
TeleCode listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `telecode.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `telecode.json`):
