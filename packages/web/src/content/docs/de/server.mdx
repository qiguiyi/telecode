---
title: Server
description: Nutze den telecode-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Der Befehl `telecode serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein telecode-Client nutzen kann.
...
Setze `TELECODE_SERVER_PASSWORD`, um den Server mit HTTP Basic Auth zu schuetzen.
Der Benutzername ist standardmaessig `telecode`, kann aber mit `TELECODE_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `telecode serve` und `telecode web`.
...
Wenn du `telecode` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den telecode-Server, um telecode programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `telecode serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `telecode serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die TeleCode-[IDE](/docs/ide)-Plugins.
...
Der telecode-Server stellt folgende APIs bereit.
