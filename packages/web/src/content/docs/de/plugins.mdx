---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um TeleCode zu erweitern.
---

Mit Plugins können Sie TeleCode erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von TeleCode zu ändern.
...
- `.telecode/plugins/` – Plugins auf Projektebene
- `~/.config/telecode/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/telecode/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/telecode/telecode.json`)
2. Projektkonfiguration (`telecode.json`)
3. Globales Plugin-Verzeichnis (`~/.config/telecode/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.telecode/plugins/`)
...
TeleCode führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein TeleCode-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie TeleCode erweitern können.
...
:::note
Wenn Sie die TeleCode-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass TeleCode `.env`-Dateien liest:
...
Plugins können TeleCode auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das TeleCode aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für TeleCode zur Verfügung.
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://telecode.ai/docs/sdk).
