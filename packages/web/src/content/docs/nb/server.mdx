---
title: Tjener
description: Samhandle med TeleCode-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `telecode serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en TeleCode-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `telecode.local` |
...
Sett `TELECODE_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `telecode` som standard, eller sett `TELECODE_SERVER_USERNAME` for å overstyre det. Dette gjelder både `telecode serve` og `telecode web`.
...
Når du kjører `telecode` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk TeleCode-serveren til å samhandle med TeleCode programmatisk.
:::

Denne arkitekturen lar TeleCode støtte flere klienter og lar deg samhandle med TeleCode programmatisk.

Du kan kjøre `telecode serve` for å starte en frittstående server. Hvis du har
TeleCode TUI kjører, vil `telecode serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av TeleCode [IDE](/docs/ide) plugins.
...
telecode-serveren viser følgende APIer.
