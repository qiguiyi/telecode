...
Du kan legge til eksterne verktøy i TeleCode ved å bruke _Model Context Protocol_ eller MCP. TeleCode støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [TeleCode Config](https://telecode.ai/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="telecode.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/telecode`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
TeleCode håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil TeleCode:
...
Hvis serveren krever autentisering, vil TeleCode be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `telecode mcp auth <server-name>`.
...
```bash
telecode mcp auth my-oauth-server
```
...
```bash
telecode mcp logout my-oauth-server
```

Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil TeleCode lagre tokens sikkert i `~/.local/share/telecode/mcp-auth.json`.
...
telecode mcp debug my-oauth-server
```
...
MCP-ene dine er tilgjengelige som verktøy i TeleCode, sammen med innebygde verktøy. Så du kan administrere dem gjennom TeleCode-konfigurasjonen som alle andre verktøy.
...
```json title="telecode.json" {11, 14-18}
{
  "$schema": "https://telecode.ai/config.json",
  "mcp": {
    "my-mcp": {
...
```bash
telecode mcp auth sentry
```

Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble TeleCode til Sentry-kontoen din.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver TeleCode-samtale oppført som en separat økt.
