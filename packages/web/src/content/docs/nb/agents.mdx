...
Det er to typer agenter i TeleCode; primære agenter og underagenter.
...
TeleCode kommer med to innebygde primære agenter, **Build** og **Plan**. Vi ser nærmere på disse nedenfor.
...
TeleCode kommer med tre innebygde underagenter, **General**, **Explore** og **Scout**. Vi skal se på dette nedenfor.

---

## Innebygd

TeleCode kommer med to innebygde primære agenter og tre innebygde underagenter.
...
Konfigurer agenter i konfigurasjonsfilen `telecode.json`:
...
Hvis ingen temperatur er spesifisert, bruker TeleCode modellspesifikke standardinnstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne banen er i forhold til hvor konfigurasjonsfilen er plassert. Så dette fungerer for både den globale TeleCode-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Modellen ID i TeleCode-konfigurasjonen din bruker formatet `provider/model-id`. For eksempel, hvis du bruker [TeleCode Zen](/docs/zen), vil du bruke `telecode/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Kjør `telecode models` for å se en liste over tilgjengelige modeller.
:::
...
:::tip
Har du en agent du vil dele? [Send inn en PR](https://github.com/anomalyco/telecode).
:::
