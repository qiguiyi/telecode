...
Agentferdigheter lar TeleCode oppdage gjenbrukbare instruksjoner fra repo- eller hjemmekatalogen din.
...
telecode søker etter disse stedene:

- Prosjektkonfigurasjon: `.telecode/skills/<name>/SKILL.md`
- Global konfigurasjon: `~/.config/telecode/skills/<name>/SKILL.md`
...
For prosjektlokale stier går telecode opp fra din nåværende arbeidskatalog til den når git-arbeidstreet.
Den laster alle matchende `skills/*/SKILL.md` i `.telecode/` og alle matchende `.claude/skills/*/SKILL.md` eller `.agents/skills/*/SKILL.md` underveis.

Globale definisjoner lastes også inn fra `~/.config/telecode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` og `~/.agents/skills/*/SKILL.md`.
...
Lag `.telecode/skills/git-release/SKILL.md` slik:
...
telecode viser tilgjengelige ferdigheter i `skill`-verktøybeskrivelsen.
...
Kontroller hvilke ferdigheter agenter har tilgang til ved å bruke mønsterbaserte tillatelser i `telecode.json`:
...
**For innebygde agenter** (i `telecode.json`):
