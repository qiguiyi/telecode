---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i TeleCode.
---

import config from "../../../../config.mjs"
export const console = config.console

TeleCode bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i TeleCode-konfigurasjonen.
...
i `~/.local/share/telecode/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i TeleCode
konfig.
...
TeleCode Zen er en liste over modeller levert av TeleCode-teamet som har vært
testet og verifisert for å fungere godt med TeleCode. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med TeleCode Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg `TeleCode Zen` og gå til [telecode.ai/auth](https://telecode.ai/zen).
...
Det fungerer som alle andre leverandører i TeleCode og er helt valgfritt å bruke.

---

## TeleCode Go

TeleCode Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av TeleCode-teamet som har vært
testet og verifisert for å fungere godt med TeleCode.

1. Kjør kommandoen `/connect` i TUI, velg `TeleCode Go`, og gå til [telecode.ai/auth](https://telecode.ai/zen).
...
Slik bruker du Amazon Bedrock med TeleCode:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører TeleCode:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `telecode.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i telecode støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere telecode til å bruke lokale modeller via [Atomic Chat](https://atomic.chat) — et skrivebordsprogram som kjører lokale LLM-er bak en OpenAI-kompatibel API-server (standard endepunkt `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at TeleCode skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom TeleCode-konfigurasjonen.
...
for å lagre token i TeleCode auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
TeleCode bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse TeleCode
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`telecode.json` fil. Det anbefales også å deaktivere øktdeling.
...
Slik bruker du GitHub Copilot-abonnementet med TeleCode:
...
Slik bruker du Google Vertex AI med TeleCode:
...
2. Angi de nødvendige miljøvariablene:
   ...
   Sett dem mens du kjører TeleCode.
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom TeleCode, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [telecode-helicone-session](https://github.com/H2Shami/telecode-helicone-session) plugin for automatisk å logge hver TeleCode-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver TeleCode-samtale oppført som en separat økt.
...
Du kan konfigurere TeleCode for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere TeleCode for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere TeleCode for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for TeleCode. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/telecode) for detaljer.
:::
...
Slik bruker du Ollama Cloud med TeleCode:
...
3. Kopier API-nøkkelen for bruk i TeleCode.
...
6. **Viktig**: Før du bruker skymodeller i TeleCode, må du hente modellinformasjonen lokalt:
...
TeleCode Zen er en liste over testede og verifiserte modeller levert av TeleCode-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>TeleCode Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **TeleCode Zen**.

   ```txt
   /connect
   ```

3. Skriv inn TeleCode API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom TeleCode-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom TeleCode-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med TeleCode:
...
Du kan også tilpasse modeller gjennom TeleCode-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med TeleCode. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `telecode.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar TeleCode forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `telecode auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk TeleCode-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i TeleCode-konfigurasjonen.
