...
description: Skriv dine egne programtillegg for å utvide TeleCode.
---

Plugins lar deg utvide TeleCode ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til TeleCode.
...
- `.telecode/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/telecode/node_modules/`.
...
1. Global konfigurasjon (`~/.config/telecode/telecode.json`)
2. Prosjektkonfigurasjon (`telecode.json`)
3. Global plugin-katalog (`~/.config/telecode/plugins/`)
4. Prosjektpluginkatalog (`.telecode/plugins/`)
...
TeleCode kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En TeleCode SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide TeleCode.
...
:::note
Hvis du bruker TeleCode-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre TeleCode fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til TeleCode:
...
`tool`-hjelperen lager et tilpasset verktøy som TeleCode kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for TeleCode sammen med innebygde verktøy.
...
Nivåer: `debug`, `info`, `warn`, `error`. Se [SDK dokumentasjon](https://telecode.ai/docs/sdk) for detaljer.
