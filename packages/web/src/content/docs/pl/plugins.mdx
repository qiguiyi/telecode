---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć telecode.
---

Wtyczki umożliwiające wyłączenie telecode poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania telecode.
...
- `.telecode/plugins/` - Wtyczki na poziomie projektu
- `~/.config/telecode/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/telecode/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/telecode/telecode.json`)
2. Konfiguracja projektu (`telecode.json`)
3. Globalny katalog wtyczek (`~/.config/telecode/plugins/`)
4. Katalog wtyczek projektu (`.telecode/plugins/`)
...
telecode uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z telecode do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do telecode.
...
:::note
Jeśli korzystasz z aplikacji komputerowej telecode, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj telecode czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do telecode:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod telecode. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
...
Poziomy: `debug`, `info`, `warn`, `error`. Aby uzyskać szczegółowe informacje, zobacz [dokumentację pakietu SDK](https://telecode.ai/docs/sdk).
