...
Dzięki telecode możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie telecode używa naszego własnego motywu `telecode`.
...
telecode ma kilka wbudowanych motywów.
...
- Chcą, aby TeleCode pasował do wyglądu ich terminala
...
telecode obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/telecode/themes/*.json` lub `$XDG_CONFIG_HOME/telecode/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.telecode/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.telecode/themes/*.json`
