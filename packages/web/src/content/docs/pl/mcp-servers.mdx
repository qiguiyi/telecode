...
Możesz dodać dodatkowe informacje do telecode za pomocą _Model Context Protocol_, w skrócie MCP. telecode obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [telecode Config](https://telecode.ai/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/telecode`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
telecode automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, telecode:
...
Jeśli serwer wymaga uwierzytelnienia, telecode poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `telecode mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji telecode bezpieczne przechowa tokeny w `~/.local/share/telecode/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w telecode, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie telecode, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć telecode z kontem Sentry.
