---
title: Serwer
description: Interakcja z serwerem telecode za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `telecode serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient telecode.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `telecode.local` |
...
Ustaw `TELECODE_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `telecode` lub ustaw `TELECODE_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `telecode serve`, jak i `telecode web`.
...
Kiedy uruchomisz `telecode`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera telecode do programowej interakcji z telecode.
:::

Ta architektura umożliwia obsługę wielu klientów przez telecode i programową interakcję z kodem otwartym.

Możesz uruchomić `telecode serve`, aby uruchomić samodzielny serwer. Jeśli masz
telecode TUI działa, `telecode serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki telecode [IDE](/docs/ide).
...
Serwer telecode udostępnia następujące interfejsy API.
