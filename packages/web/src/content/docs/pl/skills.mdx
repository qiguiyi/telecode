...
Umiejętności agenta pozwalają telecode odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
telecode przeszukuje te lokalizacje:

- Project config: `.telecode/skills/<name>/SKILL.md`
- Global config: `~/.config/telecode/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu telecode przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.telecode/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/telecode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.telecode/skills/git-release/SKILL.md` w ten sposób:
...
telecode wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `telecode.json`:
...
**Dla agentów wbudowanych** (w `telecode.json`):
