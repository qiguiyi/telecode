...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. TeleCode posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w TeleCode.
...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `TELECODE_EXPERIMENTAL_LSP_TOOL=true` (lub `TELECODE_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy TeleCode lub gdy zmienna środowiskowa `TELECODE_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu TeleCode:
