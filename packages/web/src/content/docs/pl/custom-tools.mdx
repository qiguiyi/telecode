...
Narzędzia specjalistyczne to funkcje, które tworzysz i które LLM może wywoływać podczas rozmów. Współpracują one z [wbudowanymi narzędziami](/docs/tools) telecode, takimi jak `read`, `write` i `bash`.
...
- Lokalnie, umieszczając je w katalogu `.telecode/tools/` swojego projektu.
- Lub globalnie, umieszczając je w `~/.config/telecode/tools/`.
