...
Aby debugować problemy z telecode, zacznij od sprawdzenia dzienników i danych lokalnych przechowywanych na dysku.
...
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\telecode\log`
...
Możesz ustawić poziom dziennika za pomocą opcji wiersza poleceń `--log-level`, aby uzyskać bardziej szczegółowe informacje debugowania. Na przykład `telecode --log-level DEBUG`.

---

## Przechowywanie danych

telecode przechowuje dane sesji i inne dane aplikacji na dysku pod adresem:

- **macOS/Linux**: `~/.local/share/telecode/`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\telecode`
...
telecode Desktop uruchamia w tle lokalny serwer telecode (sidecar `telecode-cli`). Większość problemów jest spowodowana nieprawidłowo działającą wtyczką, uszkodzoną pamięcią podręczną lub złymi ustawieniami serwera.
...
- Tylko macOS: menu `TeleCode` -> **Załaduj ponownie przeglądarkę internetową** (pomaga, jeśli interfejs użytkownika jest pusty/zawieszony).
...
- **macOS/Linux**: `~/.config/telecode/telecode.jsonc` (lub `~/.config/telecode/telecode.json`)
- **macOS/Linux** (starsze instalacje): `~/.local/share/telecode/telecode.jsonc`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\telecode\telecode.jsonc`
...
telecode może także ładować lokalne wtyczki z dysku. Tymczasowo usuń je (lub zmień nazwę folderu) i uruchom ponownie aplikację komputerową:

- **Wtyczki globalne**
  - **macOS/Linux**: `~/.config/telecode/plugins/`
  - **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\telecode\plugins`
...
Jeśli wyłączenie wtyczek nie pomoże (lub instalacja wtyczki utknęła), wyczyść pamięć podręczną, aby telecode mógł ją odbudować.

1. Całkowicie zamknij telecode Desktop.
2. Usuń katalog pamięci podręcznej:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/telecode`
- **Linux**: usuń `~/.cache/telecode` (lub uruchom `rm -rf ~/.cache/telecode`)
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.cache\telecode`

3. Uruchom ponownie telecode Desktop.

---

### Napraw problemy z połączeniem z serwerem

telecode Desktop może uruchomić własny serwer lokalny (domyślnie) lub połączyć się ze skonfigurowanym adresem URL serwera.
...
Jeśli Twój `telecode.json(c)` zawiera sekcję `server`, tymczasowo usuń ją i uruchom ponownie aplikację komputerową.

#### Sprawdź zmienne środowiskowe

Jeśli w swoim środowisku masz ustawiony `TELECODE_PORT`, aplikacja komputerowa spróbuje użyć tego portu dla serwera lokalnego.

- usuń `TELECODE_PORT` (lub wybierz wolny port) i uruchom ponownie.
...
W systemie Windows telecode Desktop wymaga Microsoft Edge **WebView2 Runtime**. Jeśli aplikacja otwiera się w pustym oknie lub nie uruchamia się, zainstaluj/zaktualizuj WebView2 i spróbuj ponownie.

---

### Windows: Ogólne problemy z wydajnością

Jeśli doświadczasz niskiej wydajności, problemów z dostępem do plików lub problemów z terminalem w systemie Windows, spróbuj użyć [WSL (podsystem Windows dla systemu Linux) (./windows-wsl). WSL zapewnia środowisko Linux, które płynniej współpracuje z funkcjami telecode.

---

### Brak powiadomień

telecode Desktop pokazuje powiadomienia systemowe tylko wtedy, gdy:

- powiadomienia są włączone dla telecode w ustawieniach systemu operacyjnego, oraz
...
1. Zamknij całkowicie telecode Desktop.
2. Znajdź i usuń te pliki (znajdują się w katalogu danych aplikacji telecode Desktop):

- `telecode.settings.dat` (domyślny adres URL serwera na komputerze stacjonarnym)
- `telecode.global.dat` i `telecode.workspace.*.dat` (stan interfejsu użytkownika, taki jak najnowsze serwery/projekty)
...
Jeśli masz problemy z telecode:
...
3. Upewnij się, że masz najnowszą wersję z `telecode upgrade`
...
Aby dowiedzieć się, do jakich modeli masz dostęp, uruchom `telecode models`
...
2. Jeśli problem będzie się powtarzał, spróbuj wyczyścić zapisaną konfigurację:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.local\share\telecode`
...
Jeśli napotkasz błędy wywołań API, może to wynikać z nieaktualnych pakietów dostawców. telecode dynamicznie instaluje pakiety dostawców (OpenAI, Anthropic, Google itp.) w razie potrzeby i przechowuje je lokalnie w pamięci podręcznej.
...
1. Wyczyść pamięć podręczną pakietu dostawcy:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.cache\telecode`

2. Uruchom ponownie kod telecode, aby ponownie zainstalować najnowsze pakiety dostawców

Zmusi to telecode do pobrania najnowszych wersji pakietów dostawców, co często rozwiązuje problemy ze zgodnością z parametrami modelu i zmianami API.
...
telecode wykryje, czy używasz Waylanda i wolisz `wl-clipboard`, w przeciwnym razie spróbuje znaleźć narzędzia schowka w kolejności: `xclip` i `xsel`.
