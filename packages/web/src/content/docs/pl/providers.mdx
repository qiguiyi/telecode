---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w telecode.
---

import config from "../../../../config.mjs"
export const console = config.console

telecode używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji telecode.
...
w `~/.local/share/telecode/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim telecode
config.
...
TeleCode Zen to lista modeli dostarczonych przez zespół telecode, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z telecode. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od TeleCode Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `TeleCode Zen` i przejdź do [telecode.ai/auth](https://telecode.ai/zen).
...
Działa jak każdy inny dostawca w telecode i jest całkowicie opcjonalny w użyciu.

---

## TeleCode Go

TeleCode Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół telecode, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z telecode.

1. Uruchom polecenie `/connect` w TUI, wybierz `TeleCode Go` i przejdź do [telecode.ai/auth](https://telecode.ai/zen).
...
Aby używać Amazon Bedrock z telecode:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania telecode:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `telecode.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w telecode nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
Możesz skonfigurować telecode do korzystania z modeli lokalnych przez [Atomic Chat](https://atomic.chat) — aplikację desktopową uruchamiającą lokalne LLM-y za serwerem API zgodnym z OpenAI (domyślny endpoint `http://127.0.0.1:1337/v1`).
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod telecode działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji telecode.
...
telecode używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować telecode
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`telecode.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Aby korzystać z subskrypcji GitHub Copilot z kodem telecode:
...
Aby używać Google Vertex AI z telecode:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania telecode.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą telecode, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [telecode-helicone-session](https://github.com/H2Shami/telecode-helicone-session), aby automatycznie rejestrować każdą konwersację telecode jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja telecode będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować telecode tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować telecode tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować telecode tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem telecode. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/telecode).
:::
...
Aby korzystać z Ollama Cloud z telecode:
...
3. Skopiuj klucz API do użycia w telecode.
...
6. **Ważne**: Przed użyciem modeli chmurowych w telecode musisz pobrać informacje o modelu lokalnie:
...
TeleCode Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół telecode. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>TeleCode Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **TeleCode Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API telecode.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji telecode.
...
5. Możesz je także dostosować za pomocą konfiguracji telecode. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z telecode:
...
Możesz także dostosować modele za pomocą konfiguracji telecode. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z telecode. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `telecode.json` w katalogu projektu:
...
Pola `limit` pozwalają telecode zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `telecode auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację telecode i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji telecode.
