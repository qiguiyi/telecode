---
title: الخادم
description: تفاعل مع خادم telecode عبر HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

يشغّل الأمر `telecode serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل telecode استخدامها.
...
| `--mdns-domain` | اسم نطاق مخصص لخدمة mDNS              | `telecode.local` |
...
عيّن `TELECODE_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `telecode`، أو عيّن `TELECODE_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `telecode serve` و `telecode web`.
...
عند تشغيل `telecode` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
العميل الذي يتحدث إلى الخادم. يوفّر الخادم نقطة نهاية لمواصفة OpenAPI 3.1.
وتُستخدم هذه النقطة أيضا لتوليد [SDK](/docs/sdk).

:::tip
استخدم خادم telecode للتفاعل مع telecode برمجيا.
:::

تتيح هذه البنية لـ telecode دعم عدة عملاء وتمكّنك من التفاعل مع telecode برمجيا.

يمكنك تشغيل `telecode serve` لبدء خادم مستقل. إذا كانت واجهة telecode في terminal (TUI)
قيد التشغيل، فسيبدأ `telecode serve` خادما جديدا.
...
يمكن استخدام نقطة النهاية [`/tui`](#tui) للتحكم في الـ TUI عبر الخادم. على سبيل المثال، يمكنك تعبئة الموجّه مسبقا أو تشغيله. يُستخدم هذا الإعداد بواسطة ملحقات TeleCode لـ [IDE](/docs/ide).
...
يعرض خادم telecode واجهات API التالية.
