---
title: المزوّدون
description: استخدام أي موفّر LLM في TeleCode.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم TeleCode ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات TeleCode.
...
في `~/.local/share/telecode/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات TeleCode.
...
TeleCode Zen هي قائمة نماذج يوفّرها فريق TeleCode وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع TeleCode. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع TeleCode Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر telecode، ثم انتقل إلى [telecode.ai/auth](https://telecode.ai/auth).
...
يعمل مثل أي مزوّد آخر في TeleCode واستخدامه اختياري بالكامل.

---

## TeleCode Go

TeleCode Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق TeleCode، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع TeleCode.

1. شغّل الأمر `/connect` في TUI، واختر `TeleCode Go`، ثم انتقل إلى [telecode.ai/auth](https://telecode.ai/zen).
...
لاستخدام Amazon Bedrock مع TeleCode:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل telecode:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `telecode.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل TeleCode غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
يمكنك تكوين telecode لاستخدام النماذج المحلية عبر [Atomic Chat](https://atomic.chat)، وهو تطبيق سطح مكتب يقوم بتشغيل LLMs المحلية خلف خادم API متوافق مع OpenAI (نقطة النهاية الافتراضية `http://127.0.0.1:1337/v1`).
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل telecode بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات telecode.
...
تخزين الرمز في مساحة تخزين مصادقة telecode.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم TeleCode نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر TeleCode
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`telecode.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
لاستخدام اشتراك GitHub Copilot مع telecode:
...
لاستخدام Google Vertex AI مع TeleCode:
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر telecode، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [telecode-helicone-session](https://github.com/H2Shami/telecode-helicone-session) لتسجيل كل محادثة TeleCode تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة TeleCode مدرجة كجلسة مستقلة.
...
يمكنك ضبط telecode لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط telecode لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط telecode لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع TeleCode. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/telecode) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع TeleCode:
...
3. انسخ مفتاح API لاستخدامه في TeleCode.
...
6. **مهم**: قبل استخدام نماذج السحابة في TeleCode، يجب سحب معلومات النموذج محليا:
...
TeleCode Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق TeleCode. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>TeleCode Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **TeleCode Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ TeleCode.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات telecode.
...
5. يمكنك أيضا تخصيصها عبر إعدادات telecode. إليك مثالا على تحديد مزوّد
...
لاستخدام [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) مع TeleCode:
...
يمكنك أيضا تخصيص النماذج عبر إعدادات telecode. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع telecode. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `telecode.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ TeleCode بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `telecode auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات telecode وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات telecode.
