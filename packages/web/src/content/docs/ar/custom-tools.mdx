...
description: أنشئ أدوات يمكن لـ LLM استدعاؤها في telecode.
---

الأدوات المخصصة هي دوال تنشئها يمكن لـ LLM استدعاؤها أثناء المحادثات. تعمل جنبا إلى جنب مع [الأدوات المدمجة](/docs/tools) في telecode مثل `read` و`write` و`bash`.
...
- محليا بوضعها في مجلد `.telecode/tools/` داخل مشروعك.
- أو عالميا بوضعها في `~/.config/telecode/tools/`.
