...
مع TeleCode يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم TeleCode سمتنا `telecode`.
...
يأتي TeleCode مع عدة سمات مدمجة.
...
- يريدون أن يطابق TeleCode مظهر terminal لديهم
...
يدعم TeleCode نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/telecode/themes/*.json` أو `$XDG_CONFIG_HOME/telecode/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.telecode/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.telecode/themes/*.json`
