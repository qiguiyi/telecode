---
title: Servidor
description: Interaja com o servidor telecode via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `telecode serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente telecode pode usar.
...
```bash
telecode serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `telecode.local` |
| `--cors`        | Origens adicionais de navegador a permitir        | `[]`             |

`--cors` pode ser passado várias vezes:

```bash
telecode serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Defina `TELECODE_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `telecode`, ou defina `TELECODE_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `telecode serve` quanto ao `telecode web`.

```bash
TELECODE_SERVER_PASSWORD=your-password telecode serve
```
...
Quando você executa `telecode`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor telecode para interagir com o telecode programaticamente.
:::

Essa arquitetura permite que o telecode suporte múltiplos clientes e permite que você interaja com o telecode programaticamente.

Você pode executar `telecode serve` para iniciar um servidor autônomo. Se você tiver o TUI do telecode em execução, `telecode serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do telecode [IDE](/docs/ide).
...
O servidor telecode expõe as seguintes APIs.
