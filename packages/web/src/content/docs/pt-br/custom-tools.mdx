...
description: Crie ferramentas que o LLM pode chamar no telecode.
---

Ferramentas personalizadas são funções que você cria e que o LLM pode chamar durante as conversas. Elas funcionam junto com as [ferramentas integradas](/docs/tools) do telecode, como `read`, `write` e `bash`.
...
- Localmente, colocando-as no diretório `.telecode/tools/` do seu projeto.
- Ou globalmente, colocando-as em `~/.config/telecode/tools/`.
