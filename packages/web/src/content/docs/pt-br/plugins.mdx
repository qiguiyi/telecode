---
title: Plugins
description: Escreva seus próprios plugins para estender o telecode.
---

Plugins permitem que você estenda o telecode conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do telecode.
...
- `.telecode/plugins/` - Plugins em nível de projeto
- `~/.config/telecode/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/telecode/node_modules/`.
...
1. Configuração global (`~/.config/telecode/telecode.json`)
2. Configuração do projeto (`telecode.json`)
3. Diretório de plugins global (`~/.config/telecode/plugins/`)
4. Diretório de plugins do projeto (`.telecode/plugins/`)
...
O telecode executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do telecode para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o telecode.
...
:::note
Se você estiver usando o aplicativo desktop telecode, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o telecode de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao telecode:
...
O helper `tool` cria uma ferramenta personalizada que o telecode pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o telecode junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://telecode.ai/docs/sdk) para detalhes.
