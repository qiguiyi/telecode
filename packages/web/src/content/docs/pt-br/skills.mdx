...
As habilidades do agente permitem que o telecode descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O telecode pesquisa nesses locais:

- Configuração do projeto: `.telecode/skills/<name>/SKILL.md`
- Configuração global: `~/.config/telecode/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o telecode sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.telecode/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/telecode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.telecode/skills/git-release/SKILL.md` assim:
...
O telecode lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `telecode.json`:
...
**Para agentes embutidos** (em `telecode.json`):
