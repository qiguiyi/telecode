...
Existem dois tipos de agentes no telecode; agentes primários e subagentes.
...
telecode vem com dois agentes primários integrados, **Build** e **Plan**. Vamos
ver isso abaixo.
...
TeleCode vem com três subagentes integrados, **General**, **Explore** e **Scout**. Vamos ver isso abaixo.

---

## Integrados

TeleCode vem com dois agentes primários integrados e três subagentes integrados.
...
Um agente somente leitura para pesquisa em documentação externa e dependências. Use-o quando você precisar clonar o repositório de uma dependência para o cache gerenciado do TeleCode, inspecionar o código-fonte de uma biblioteca ou cruzar o código local com implementações upstream sem modificar seu workspace.
...
Configure os agentes em seu arquivo de configuração `telecode.json`:
...
- Global: `~/.config/telecode/agents/`
- Por projeto: `.telecode/agents/`

```markdown title="~/.config/telecode/agents/review.md"
---
...
Se nenhuma temperatura for especificada, o telecode usa padrões específicos do modelo; tipicamente 0 para a maioria dos modelos, 0.55 para modelos Qwen.
...
Este caminho é relativo ao local onde o arquivo de configuração está localizado. Portanto, isso funciona tanto para a configuração global do telecode quanto para a configuração específica do projeto.
...
O ID do modelo em sua configuração do telecode usa o formato `provider/model-id`. Por exemplo, se você estiver usando [TeleCode Zen](/docs/zen), você usaria `telecode/gpt-5.1-codex` para GPT 5.1 Codex.
...
```json title="telecode.json" {3-6,9-12}
{
  "$schema": "https://telecode.ai/config.json",
  "tools": {
    "write": true,
...
```json title="telecode.json" {3-5,8-10}
{
  "$schema": "https://telecode.ai/config.json",
  "permission": {
    "edit": "deny"
...
:::tip
Execute `telecode models` para ver uma lista dos modelos disponíveis.
:::
...
:::tip
Você tem um agente que gostaria de compartilhar? [Envie um PR](https://github.com/anomalyco/telecode).
:::
...
```markdown title="~/.config/telecode/agents/docs-writer.md"
---
...
```markdown title="~/.config/telecode/agents/security-auditor.md"
---
