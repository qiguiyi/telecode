...
Com o telecode, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o telecode usa nosso próprio tema `telecode`.
...
O telecode vem com vários temas embutidos.
...
- Querem que o telecode corresponda à aparência do seu terminal
...
O telecode suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/telecode/themes/*.json` ou `$XDG_CONFIG_HOME/telecode/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.telecode/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.telecode/themes/*.json`
