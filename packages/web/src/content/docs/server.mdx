---
title: Server
description: Interact with telecode server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `telecode serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an telecode client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `telecode.local` |
...
Set `TELECODE_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `telecode`, or set `TELECODE_SERVER_USERNAME` to override it. This applies to both `telecode serve` and `telecode web`.
...
When you run `telecode` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the telecode server to interact with telecode programmatically.
:::

This architecture lets telecode support multiple clients and allows you to interact with telecode programmatically.

You can run `telecode serve` to start a standalone server. If you have the
telecode TUI running, `telecode serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the TeleCode [IDE](/docs/ide) plugins.
...
The telecode server exposes the following APIs.
