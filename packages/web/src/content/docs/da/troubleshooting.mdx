...
For at fejlfinde problemer med TeleCode, start med at tjekke logfilerne og de lokale data, den gemmer på disken.
...
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\telecode\log`

Logfiler navngives med tidsstempler (f.eks. `2025-01-09T123456.log`) og de sidste 10 logfiler beholdes.

Du kan angive logniveauet med kommandolinjeflaget `--log-level` for at få mere detaljeret fejlfindingsinformation. For eksempel `telecode --log-level DEBUG`.

---

## Lagring

telecode gemmer sessionsdata og andre applikationsdata på disken på:

- **macOS/Linux**: `~/.local/share/telecode/`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\telecode`
...
TeleCode Desktop kører en lokal TeleCode-server (`telecode-cli` sidevognen) i baggrunden. De fleste problemer er forårsaget af et plugin, der fungerer dårligt, en ødelagt cache eller en dårlig serverindstilling.

### Hurtige tjek

- Afslut og start appen på ny.
- Hvis appen viser en fejlskærm, klikker du på **Start på ny** og kopierer fejldetaljerne.
- Kun macOS: `TeleCode`-menu -> **Genindlæs webvisning** (hjælper hvis UI er tom/frosset).
...
- **macOS/Linux**: `~/.config/telecode/telecode.jsonc` (eller `~/.config/telecode/telecode.json`)
- **macOS/Linux** (ældre installationer): `~/.local/share/telecode/telecode.jsonc`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\telecode\telecode.jsonc`
...
TeleCode kan også indlæse lokale plugins fra disken. Flyt disse midlertidigt væk (eller giv mappen nyt navn) og start skrivebordsappen på ny:

- **Globale plugins**
  - **macOS/Linux**: `~/.config/telecode/plugins/`
  - **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\telecode\plugins`
...
Hvis deaktivering af plugins ikke hjælper (eller en plugin-installation sidder fast), tøm cachen så TeleCode kan genopbygge den.

1. Afslut TeleCode Desktop helt.
2. Slet cache-mappen:

- **macOS**: Finder -> `Cmd+Shift+G` -> indsæt `~/.cache/telecode`
- **Linux**: slet `~/.cache/telecode` (eller kør `rm -rf ~/.cache/telecode`)
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.cache\telecode`

3. Start TeleCode Desktop på ny.

---

### Løs problemer med serverforbindelse

TeleCode Desktop kan enten starte sin egen lokale server (standard) eller forbinde til en server URL, du har konfigureret.
...
Hvis `telecode.json(c)` indeholder en `server`-del, fjern den midlertidigt og start skrivebordsappen på ny.

#### Tjek miljøvariabler

Hvis du har `TELECODE_PORT` sat i dit miljø, vil skrivebordsappen prøve at bruge den port for den lokale server.

- Deaktiver `TELECODE_PORT` (eller vælg en ledig port) og start på ny.
...
På Windows kræver TeleCode Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åbnes i et tomt vindue eller ikke starter, installer/opdater WebView2 og prøv igen.

---

### Windows: Generelle ydeevneproblemer

Hvis du oplever langsom ydeevne, problemer med filadgang eller terminalproblemer på Windows, kan du prøve at bruge [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL giver et Linux-miljø som fungerer mere sømløst med funktionerne i TeleCode.

---

### Meddelelser vises ikke

TeleCode Desktop viser kun systemvarsler når:

- varsler er aktiveret for TeleCode i dine OS-indstillinger, og
...
1. Afslut TeleCode Desktop.
2. Find og slet disse filer (de findes i TeleCode Desktop-appens datamappe):

- `telecode.settings.dat` (skrivebordsstandardserver URL)
- `telecode.global.dat` og `telecode.workspace.*.dat` (UI tilstand som nylige servere/projekter)
...
Hvis du oplever problemer med TeleCode:
...
For at finde ud af hvilke modeller du har adgang til, kør `telecode models`
...
2. Hvis problemet vedvarer, prøv at tømme den gemte konfiguration:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.local\share\telecode`
...
Hvis du støder på API-kaldsfejl, kan dette skyldes forældede udbyderpakker. telecode installerer dynamisk udbyderpakker (OpenAI, Anthropic, Google, etc.) efter behov og cacher dem lokalt.
...
1. Tøm udbyderens pakkecache:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.cache\telecode`

2. Start telecode på ny for at installere de nyeste udbyderpakker på ny

Dette vil tvinge telecode til at downloade de nyeste versioner af udbyderpakkerne, som ofte løser kompatibilitetsproblemer med modelparametre og API-ændringer.
...
telecode vil opdage om du bruger Wayland og foretrække `wl-clipboard`, ellers vil den prøve at finde udklipsholderværktøjer i rækkefølgen: `xclip` og `xsel`.
