...
Med TeleCode kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger TeleCode vårt eget `telecode`-tema.
...
TeleCode kommer med flere innebygde temaer.
...
- Vil at TeleCode skal matche terminalens utseende
...
TeleCode støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/telecode/themes/*.json` eller `$XDG_CONFIG_HOME/telecode/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.telecode/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.telecode/themes/*.json`
