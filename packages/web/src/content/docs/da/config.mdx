---
title: Konfiguration
description: Ved at bruge TeleCode JSON konfig.
---

Du kan konfigurere TeleCode ved hjælp af en JSON-konfigurationsfil.

---

## Format

TeleCode understøtter både **JSON** og **JSONC** (JSON med kommentarer) formater.
...
1. **Fjernkonfiguration** (fra `.well-known/telecode`) - organisatoriske standardindstillinger
2. **Global config** (`~/.config/telecode/telecode.json`) - brugerpræferencer
3. **Tilpasset konfiguration** (`TELECODE_CONFIG` env var) - tilpassede tilsidesættelser
4. **Project config** (`telecode.json` i projekt) - projektspecifikke indstillinger
5. **`.telecode` mapper** - agent, kommandoer, plugins
6. **Inline config** (`TELECODE_CONFIG_CONTENT` env var) - runtime tilsidesættelser
...
:::note
`.telecode` og `~/.config/telecode` mapperne bruger **flertalsnavne** for undermapper: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` og `themes/`. Enkelte navne (f.eks. `agent/`) understøtter også bagudkompatibilitet.
:::
...
Organisationer kan levere standardkonfiguration via `.well-known/telecode`-slutpunktet. Dette hentes automatisk, når du godkender med en udbyder, der understøtter det.
...
Placer din globale TeleCode-konfiguration i `~/.config/telecode/telecode.json`. Brug global konfiguration til brugerdækkende præferencer som temaer, udbydere eller nøglebindinger.
...
Tilføj `telecode.json` i dit projektrod. Project config har den højeste forrang blandt standard config-filer - den tilsidesætter både globale og eksterne config.

:::tip
Placer projektspecifik konfiguration i roden af ​​dit projekt.
:::

Når TeleCode starter op, søger den efter en konfigurationsfil i den aktuelle mappe eller går op til den nærmeste Git-mappe.
...
Angiv en brugerdefineret konfigurationsfilsti ved hjælp af miljøvariablen `TELECODE_CONFIG`.
...
Angiv en brugerdefineret konfigurationsmappe ved hjælp af `TELECODE_CONFIG_DIR`
miljøvariabel. Dette kort vil blive søgt efter agenter, kommandoer,
modes og plugins ligesom standard `.telecode` mappen, og bør
...
Den brugerdefinerede mappe indlæses efter den globale konfig og `.telecode` mapper, så den **kan tilsidesætte** deres indstillinger.

---

## Skema

Konfigurationsfilen har et skema, der defineres i [**`telecode.ai/config.json`**](https://telecode.ai/config.json).
...
Du kan konfigurere serverindstillinger for kommandoerne `telecode serve` og `telecode web` gennem indstillingen `server`.
...
- `mdns` - Aktiver mDNS-tjenesteopdagelse. Dette gør det muligt for andre enheder på netværket at opdage din TeleCode-server.
- `mdnsDomain` - Brugerdefineret domænenavn til mDNS-tjeneste. Som standard er `telecode.local`. Nyttigt til at køre flere forekomster på det samme netværk.
...
Du kan konfigurere de udbydere og modeller, du vil bruge i din TeleCode-konfiguration, gennem mulighederne `provider`, `model` og `small_model`.
...
Indstillingen `small_model` konfigurerer en separat model til lette opgaver som titelgenerering. Som standard forsøger TeleCode at bruge en billigere model, hvis en er tilgængelig fra din udbyder, ellers falder den tilbage til din hovedmodel.
...
Du kan konfigurere det tema, du vil bruge i din TeleCode-konfiguration, gennem indstillingen `theme`.
...
Du kan også definere agenter ved at bruge markdown-filer i `~/.config/telecode/agents/` eller `.telecode/agents/`. [Learn more here](/docs/agents).
...
Standardagenten skal være en primær agent (ikke en subagent). Dette kan være en indbygget agent som `"build"` eller `"plan"`, eller en [custom agent](/docs/agents), du har defineret. Hvis den angivne agent ikke eksisterer eller er en underagent, vil TeleCode falde tilbage til `"build"` med en advarsel.

Denne indstilling gælder på tværs af alle grænseflader: TUI, CLI (`telecode run`), desktop-app og GitHub Action.
...
Du kan også definere kommandoer ved hjælp af markdown-filer i `~/.config/telecode/commands/` eller `.telecode/commands/`. [Learn more here](/docs/commands).
...
TeleCode vil automatisk downloade alle nye opdateringer, når den starter op. Du kan deaktivere dette med indstillingen `autoupdate`.
...
Som standard telecode **tillader alle operationer** uden at kræve eksplicit godkendelse. Du kan ændre dette ved at bruge indstillingen `permission`.
...
[Plugins](/docs/plugins) udvide TeleCode med brugerdefinerede værktøjer, kroge og integrationer.

Placer plugin-filer i `.telecode/plugins/` eller `~/.config/telecode/plugins/`. Du kan også indlæse plugins fra npm gennem indstillingen `plugin`.
...
Dette er nyttigt, når du vil begrænse TeleCode til kun at bruge specifikke udbydere i stedet for at deaktivere dem én efter én.
