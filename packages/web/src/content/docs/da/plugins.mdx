---
title: Plugins
description: Skriv dine egne plugins for at udvide TeleCode.
---

Plugins giver dig mulighed for at udvide TeleCode ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre TeleCodes standardadfærd.
...
- `.telecode/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/telecode/node_modules/`.
...
1. Global konfiguration (`~/.config/telecode/telecode.json`)
2. Projektkonfiguration (`telecode.json`)
3. Global plugin-mappe (`~/.config/telecode/plugins/`)
4. Projekt plugin bibliotek (`.telecode/plugins/`)
...
TeleCode kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En telecode SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide telecode.
...
:::note
Hvis du bruger TeleCode desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå telecode i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til telecode:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som telecode kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for telecode sammen med indbyggede værktøjer.
...
Niveauer: `debug`, `info`, `warn`, `error`. Se [SDK documentation](https://telecode.ai/docs/sdk) for detaljer.
