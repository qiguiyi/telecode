...
Her er alle de innebygde verktøyene som er tilgængelige i TeleCode.
...
:::note
Dette verktøyet er bare tilgængelig når `TELECODE_EXPERIMENTAL_LSP_TOOL=true` (eller `TELECODE_EXPERIMENTAL=true`).
:::
...
:::note
Dette verktøyet er bare tilgængelig når du bruger TeleCode-leverandøren eller når miljøvariabelen `TELECODE_ENABLE_EXA` er satt til en sannverdi (f.eks. `true` eller `1`).
