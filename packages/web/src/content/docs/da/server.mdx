---
title: Server
description: Interager med telecode-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `telecode serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en telecode-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `telecode.local` |
...
Indstil `TELECODE_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `telecode`, eller indstil `TELECODE_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `telecode serve` og `telecode web`.
...
Når du kører `telecode` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug telecode-serveren til at interagere med telecode programmatisk.
:::

Denne arkitektur lader telecode understøtte flere klienter og lader dig interagere med telecode programmatisk.

Du kan køre `telecode serve` for at starte en selvstændig server. Hvis du har
telecode TUI kørende, vil `telecode serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af TeleCode [IDE](/docs/ide) plugins.
...
TeleCode-serveren viser følgende API'er.
