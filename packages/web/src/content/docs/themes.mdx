...
With TeleCode you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, TeleCode uses our own `telecode` theme.
...
TeleCode comes with several built-in themes.
...
- Want TeleCode to match their terminal's appearance
...
TeleCode supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/telecode/themes/*.json` or `$XDG_CONFIG_HOME/telecode/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.telecode/themes/*.json`
4. **Current working directory** - Defined in `./.telecode/themes/*.json`
