---
title: Plugins
description: Écrivez vos propres plugins pour étendre TeleCode.
---

Les plugins vous permettent d'étendre TeleCode en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de TeleCode.
...
- `.telecode/plugins/` - Plugins au niveau du projet
- `~/.config/telecode/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/telecode/node_modules/`.
...
1. Configuration globale (`~/.config/telecode/telecode.json`)
2. Configuration du projet (`telecode.json`)
3. Répertoire global des plugins (`~/.config/telecode/plugins/`)
4. Répertoire des plugins du projet (`.telecode/plugins/`)
...
TeleCode exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK telecode pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre telecode.
...
:::note
Si vous utilisez l'application de bureau TeleCode, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez telecode de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à telecode :
...
L'assistant `tool` crée un outil personnalisé que telecode peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour telecode aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://telecode.ai/docs/sdk) pour plus de détails.
