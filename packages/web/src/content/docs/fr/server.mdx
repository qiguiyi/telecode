---
title: Serveur
description: Interagissez avec le serveur telecode via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `telecode serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client telecode peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `telecode.local` |
...
Définissez `TELECODE_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `telecode`, ou définissez `TELECODE_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `telecode serve` et à `telecode web`.
...
Lorsque vous exécutez `telecode`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur telecode pour interagir avec telecode par programmation.
:::

Cette architecture permet à telecode de prendre en charge plusieurs clients et vous permet d'interagir avec telecode par programme.

Vous pouvez exécuter `telecode serve` pour démarrer un serveur autonome. Si vous avez le
telecode TUI en cours d'exécution, `telecode serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins TeleCode [IDE](/docs/ide).
...
Le serveur telecode expose les API suivantes.
