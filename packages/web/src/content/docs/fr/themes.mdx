...
Avec TeleCode, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, TeleCode utilise notre propre thème `telecode`.
...
TeleCode est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que TeleCode corresponde à l'apparence de leur terminal
...
TeleCode prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/telecode/themes/*.json` ou `$XDG_CONFIG_HOME/telecode/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.telecode/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.telecode/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/telecode/themes
vim ~/.config/telecode/themes/my-theme.json
```

Et pour des thèmes spécifiques au projet.

```bash no-frame
mkdir -p .telecode/themes
vim .telecode/themes/my-theme.json
```
...
{
  "$schema": "https://telecode.ai/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
