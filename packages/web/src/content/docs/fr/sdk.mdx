---
title: SDK
description: Client JS de type sécurisé pour le serveur telecode.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Le SDK telecode JS/TS fournit un client de type sécurisé pour interagir avec le serveur.
Utilisez-le pour créer des intégrations et contrôler telecode par programme.
...
Créez une instance de telecode :

```javascript
import { createTeleCode } from "@telecode-ai/sdk"

const { client } = await createTeleCode()
```
...
Vous pouvez transmettre un objet de configuration pour personnaliser le comportement. L'instance récupère toujours votre `telecode.json`, mais vous pouvez remplacer ou ajouter une configuration en ligne :
...
console.log(`Server running at ${telecode.server.url}`)
...
Si vous disposez déjà d'une instance en cours d'exécution de telecode, vous pouvez créer une instance client pour vous y connecter :
...
```typescript
import type { Session, Message, Part } from "@telecode-ai/sdk"
```
...
const textResults = await client.find.text({
  query: { pattern: "function.*telecode" },
})
