...
Создайте `.telecode/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию telecode или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем telecode [config](/docs/config):
...
- Глобальный: `~/.config/telecode/commands/`
- Для каждого проекта: `.telecode/commands/`
...
telecode включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
