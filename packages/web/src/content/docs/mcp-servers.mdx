...
You can add external tools to TeleCode using the _Model Context Protocol_, or MCP. TeleCode supports both local and remote servers.
...
You can define MCP servers in your [TeleCode Config](https://telecode.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/telecode` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
TeleCode automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, TeleCode will:
...
If the server requires authentication, TeleCode will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `telecode mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, TeleCode will store the tokens securely in `~/.local/share/telecode/mcp-auth.json`.
...
Your MCPs are available as tools in TeleCode, alongside built-in tools. So you can manage them through the TeleCode config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect TeleCode to your Sentry account.
