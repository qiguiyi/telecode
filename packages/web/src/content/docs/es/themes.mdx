...
Con TeleCode puede seleccionar uno de varios temas integrados, usar un tema que se adapte al tema de su terminal o definir su propio tema personalizado.

De forma predeterminada, TeleCode usa nuestro propio tema `telecode`.
...
TeleCode viene con varios temas integrados.
...
- Quiere que TeleCode coincida con la apariencia de su terminal
...
TeleCode admite un sistema de temas flexible basado en JSON que permite a los usuarios crear y personalizar temas fácilmente.
...
2. **Directorio de configuración de usuario**: definido en `~/.config/telecode/themes/*.json` o `$XDG_CONFIG_HOME/telecode/themes/*.json`
3. **Directorio raíz del proyecto**: definido en `<project-root>/.telecode/themes/*.json`
4. **Directorio de trabajo actual** - Definido en `./.telecode/themes/*.json`
