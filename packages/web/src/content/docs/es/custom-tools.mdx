...
description: Cree herramientas que LLM pueda llamar en telecode.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de telecode como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.telecode/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/telecode/tools/`.
...
```ts title=".telecode/tools/database.ts" {1}
...
```ts title=".telecode/tools/math.ts"
import { tool } from "@telecode-ai/plugin"
...
```ts title=".telecode/tools/bash.ts"
import { tool } from "@telecode-ai/plugin"
...
```ts title=".telecode/tools/project.ts" {8}
import { tool } from "@telecode-ai/plugin"
...
```python title=".telecode/tools/add.py"
import sys
...
```ts title=".telecode/tools/python-add.ts" {10}
import { tool } from "@telecode-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".telecode/tools/add.py")
