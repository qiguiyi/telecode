---
title: Complementos
description: Escriba sus propios complementos para extender TeleCode.
---

Los complementos le permiten extender TeleCode al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de TeleCode.
...
- `.telecode/plugins/` - Complementos a nivel de proyecto
- `~/.config/telecode/plugins/` - Complementos globales
...
```json title="telecode.json"
{
  ...
  "plugin": ["telecode-helicone-session", "telecode-wakatime", "@my-org/custom-plugin"]
}
```
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/telecode/node_modules/`.
...
1. Configuración global (`~/.config/telecode/telecode.json`)
2. Configuración del proyecto (`telecode.json`)
3. Directorio global de complementos (`~/.config/telecode/plugins/`)
4. Directorio de complementos del proyecto (`.telecode/plugins/`)
...
```json title=".telecode/package.json"
{
  "dependencies": {
    "shescape": "^2.1.0"
  }
...
TeleCode ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.

```ts title=".telecode/plugins/my-plugin.ts"
...
```js title=".telecode/plugins/example.js"
...
- `client`: Un cliente SDK telecode para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar telecode.
...
```js title=".telecode/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "telecode"'`
      }
    },
  }
}
...
:::note
Si está utilizando la aplicación de escritorio TeleCode, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que telecode lea archivos `.env`:

```javascript title=".telecode/plugins/env-protection.js"
...
```javascript title=".telecode/plugins/inject-env.js"
...
Los complementos también pueden agregar herramientas personalizadas a telecode:

```ts title=".telecode/plugins/custom-tools.ts"
import { type Plugin, tool } from "@telecode-ai/plugin"
...
El ayudante `tool` crea una herramienta personalizada a la que telecode puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para telecode junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://telecode.ai/docs/sdk) para obtener más detalles.
...
```ts title=".telecode/plugins/compaction.ts"
import type { Plugin } from "@telecode-ai/plugin"
...
```ts title=".telecode/plugins/custom-compaction.ts"
import type { Plugin } from "@telecode-ai/plugin"
