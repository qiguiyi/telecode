...
Las habilidades del agente permiten a TeleCode descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
TeleCode busca estas ubicaciones:

- Configuración del proyecto: `.telecode/skills/<name>/SKILL.md`
- Configuración global: `~/.config/telecode/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, TeleCode sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.telecode/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/telecode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.telecode/skills/git-release/SKILL.md` así:
...
TeleCode enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `telecode.json`:
...
**Para agentes integrados** (en `telecode.json`):
