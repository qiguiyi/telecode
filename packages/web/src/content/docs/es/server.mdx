---
title: Servidor
description: Interactuar con el servidor telecode a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `telecode serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente telecode puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `telecode.local` |
...
Configure `TELECODE_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `telecode`, o configure `TELECODE_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `telecode serve` como a `telecode web`.
...
Cuando ejecuta `telecode`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor telecode para interactuar con telecode mediante programación.
:::

Esta arquitectura permite que telecode admita múltiples clientes y le permite interactuar con telecode mediante programación.

Puede ejecutar `telecode serve` para iniciar un servidor independiente. Si tienes el
telecode TUI ejecutándose, `telecode serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos TeleCode [IDE](/docs/ide).
...
El servidor telecode expone las siguientes API.
