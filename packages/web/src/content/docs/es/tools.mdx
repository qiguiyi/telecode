...
Las herramientas permiten que LLM realice acciones en su código base. TeleCode viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en TeleCode.
...
:::note
Esta herramienta solo está disponible cuando `TELECODE_EXPERIMENTAL_LSP_TOOL=true` (o `TELECODE_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor TeleCode o cuando la variable de entorno `TELECODE_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar TeleCode:

```bash
TELECODE_ENABLE_EXA=1 telecode
```
