...
Ajan becerileri, telecode'un deponuzdan veya ev dizininizden tekrar kullanilabilir talimatlari kesfetmesini saglar.
...
telecode su konumlari tarar:

- Proje konfigurasyonu: `.telecode/skills/<name>/SKILL.md`
- Genel konfigurasyon: `~/.config/telecode/skills/<name>/SKILL.md`
...
Proje yerel yollarinda telecode, mevcut calisma dizininizden baslayip git worktree kokune kadar yukari cikar.
Bu sirada `skills/*/SKILL.md` altindaki eslesen `.telecode/` dosyalarini ve `.claude/skills/*/SKILL.md` ya da `.agents/skills/*/SKILL.md` dosyalarini yukler.

Genel tanimlar da `~/.config/telecode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` ve `~/.agents/skills/*/SKILL.md` yollarindan yuklenir.
...
`.telecode/skills/git-release/SKILL.md` dosyasini soyle olusturun:
...
telecode, `skill` araci aciklamasinda mevcut becerileri listeler.
...
`telecode.json` icinde pattern tabanli izinlerle ajanlarin hangi becerilere erisecegini kontrol edin:
...
**Yerlesik ajanlar icin** (`telecode.json` icinde):
