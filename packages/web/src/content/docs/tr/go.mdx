...
TeleCode Go, popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir aboneliktir — **ilk ayınız için 5$**, sonrasında **aylık 10$**.

Go, TeleCode'daki diğer sağlayıcılar gibi çalışır. TeleCode Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve TeleCode'u kullanmak için bunu kullanmanıza gerek yoktur.
...
TeleCode Go, bu modellere **ilk ayınız için 5$**, ardından **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır?

TeleCode Go, TeleCode'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>TeleCode Zen</a>**'e giriş yapın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `TeleCode Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanabileceğiniz modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Her çalışma alanından yalnızca bir üye TeleCode Go'ya abone olabilir.
:::
...
TeleCode Go aşağıdaki limitleri içerir:
...
TeleCode yapılandırmanızdaki [model id](/docs/config/#models) formatı `telecode-go/<model-id>` şeklindedir. Örneğin, Kimi K2.7 Code için yapılandırmanızda `telecode-go/kimi-k2.7-code` kullanmalısınız.
...
TeleCode Go'yu şu amaçlarla oluşturduk:
...
4. TeleCode ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.
