...
_Model Bağlam Protokolü_ veya MCP kullanarak telecode'a harici araçlar ekleyebilirsiniz. telecode hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [telecode Config](https://telecode.ai/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/telecode` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
telecode, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde telecode şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa telecode, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `telecode mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından telecode, belirteçleri `~/.local/share/telecode/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra telecode'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi telecode yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve telecode'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
