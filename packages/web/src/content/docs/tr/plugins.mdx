---
title: Eklentiler
description: TeleCode'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek TeleCode'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya TeleCode'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.telecode/plugins/` - Proje düzeyinde eklentiler
- `~/.config/telecode/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/telecode/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/telecode/telecode.json`)
2. Project config (`telecode.json`)
3. Global eklenti dizini (`~/.config/telecode/plugins/`)
4. Proje eklenti dizini (`.telecode/plugins/`)
...
TeleCode bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir TeleCode SDK istemcisi.
...
TeleCode'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
TeleCode masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
TeleCode'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca TeleCode'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, TeleCode'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra TeleCode'a da hazır olacaktır.
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK belgelerine](https://telecode.ai/docs/sdk) bakın.
