...
telecode ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak telecode kendi `telecode` temasını kullanır.
...
telecode birden fazla yerleşik temayla gelir.
...
- telecode'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
telecode, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/telecode/themes/*.json` veya `$XDG_CONFIG_HOME/telecode/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.telecode/themes/*.json`
4. **Geçerli çalışma dizini** - `./.telecode/themes/*.json`
