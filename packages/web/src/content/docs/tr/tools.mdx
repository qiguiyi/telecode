...
Araçlar, LLM'in kod tabanınızda eylem gerçekleştirmesini sağlar. telecode yerleşik bir araç setiyle gelir, ancak bunu [özel araçlarla](/docs/custom-tools) veya [MCP sunucularıyla](/docs/mcp-servers) genişletebilirsiniz.
...
telecode içinde bulunan tüm yerleşik araçlar aşağıdadır.
...
:::note
Bu araç yalnızca `TELECODE_EXPERIMENTAL_LSP_TOOL=true` (veya `TELECODE_EXPERIMENTAL=true`) olduğunda kullanılabilir.
:::
...
:::note
Bu araç yalnızca telecode provider kullanılırken veya `TELECODE_ENABLE_EXA` ortam değişkeni doğru değerlerden birine ayarlandığında (örneğin `true` ya da `1`) kullanılabilir.

telecode başlatırken etkinleştirmek için:
