---
title: Sunucu
description: telecode sunucusuyla HTTP uzerinden etkilesin.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`telecode serve` komutu, telecode istemcisinin kullanabileceği bir OpenAPI endpoint'i açan headless bir HTTP sunucusu çalıştırır.
...
| `--mdns-domain` | mDNS hizmeti için özel alan adı                | `telecode.local` |
...
Sunucuyu HTTP temel kimlik doğrulama (basic auth) ile korumak için `TELECODE_SERVER_PASSWORD` ayarlayın. Kullanıcı adı varsayılan olarak `telecode` değeridir; değiştirmek isterseniz `TELECODE_SERVER_USERNAME` ayarlayabilirsiniz. Bu ayar hem `telecode serve` hem de `telecode web` için geçerlidir.
...
`telecode` çalıştırdığınızda hem TUI hem de sunucu başlar. TUI, sunucuyla konuşan istemci tarafıdır.
Sunucu bir OpenAPI 3.1 spesifikasyon uç noktası açar. Bu uç nokta [SDK](/docs/sdk) üretiminde de kullanılır.

:::tip
telecode ile programatik etkileşim için telecode sunucusunu kullanın.
:::

Bu mimari, telecode'un birden fazla istemciyi desteklemesini ve programatik kullanımları mümkün kılmasını sağlar.

`telecode serve` ile bağımsız bir sunucu başlatabilirsiniz. telecode TUI açıksa, `telecode serve` yeni bir sunucu başlatır.
...
[`/tui`](#tui) uç noktası sunucu üzerinden TUI'yi sürmek için kullanılabilir. Örneğin bir istemi önceden doldurabilir veya çalıştırabilirsiniz. Bu kurulum telecode [IDE](/docs/ide) eklentileri tarafından kullanılır.
...
telecode sunucusu aşağıdaki API'leri sunar.
