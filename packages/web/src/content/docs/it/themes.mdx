...
Con TeleCode puoi scegliere tra diversi temi integrati, usare un tema che si adatta al tema del tuo terminale oppure definire un tema personalizzato.

Per impostazione predefinita, TeleCode usa il tema `telecode`.
...
TeleCode include diversi temi integrati.
...
- Vuole che TeleCode corrisponda all'aspetto del terminale
...
TeleCode supporta un sistema di temi flessibile basato su JSON che permette di creare e personalizzare temi facilmente.
...
2. **Directory di configurazione utente** - in `~/.config/telecode/themes/*.json` o `$XDG_CONFIG_HOME/telecode/themes/*.json`
3. **Directory root del progetto** - in `<project-root>/.telecode/themes/*.json`
4. **Directory di lavoro corrente** - in `./.telecode/themes/*.json`
