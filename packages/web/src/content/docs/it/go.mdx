...
TeleCode Go è un abbonamento a basso costo — **5 $ per il primo mese**, poi **10 $/mese** — che ti offre un accesso affidabile ai popolari modelli di programmazione aperti.

Go funziona come qualsiasi altro provider in TeleCode. Ti abboni a TeleCode Go e
...
TeleCode Go ti dà accesso a questi modelli a **5 $ per il primo mese**, poi a **10 $/mese**.

---

## Come funziona

TeleCode Go funziona come qualsiasi altro provider in TeleCode.

1. Accedi a **<a href={console}>TeleCode Zen</a>**, ti abboni a Go e
   copi la tua chiave API.
2. Esegui il comando `/connect` nella TUI, selezioni `TeleCode Go` e incolli
   la tua chiave API.
3. Esegui `/models` nella TUI per vedere l'elenco dei modelli disponibili tramite Go.

:::note
Solo un membro per workspace può abbonarsi a TeleCode Go.
:::
...
TeleCode Go include i seguenti limiti:
...
Il [model id](/docs/config/#models) nella tua TeleCode config
utilizza il formato `telecode-go/<model-id>`. Ad esempio, per Kimi K2.7 Code, useresti
`telecode-go/kimi-k2.7-code` nella tua configurazione.
...
Abbiamo creato TeleCode Go per:
...
4. Non avere **nessun lock-in**, permettendoti di utilizzare anche qualsiasi altro provider con TeleCode.
