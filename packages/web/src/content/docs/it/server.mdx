---
title: Server
description: Interagisci con il server telecode via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `telecode serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client telecode.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `telecode.local` |
...
Imposta `TELECODE_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `telecode`, oppure imposta `TELECODE_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `telecode serve` sia per `telecode web`.
...
Quando esegui `telecode` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server telecode per interagire con telecode in modo programmatico.
:::

Questa architettura permette a telecode di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `telecode serve` per avviare un server standalone. Se la TUI di telecode e gia in esecuzione, `telecode serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di TeleCode.
...
Il server telecode espone le seguenti API.
