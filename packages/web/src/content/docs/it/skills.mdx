...
Le skill degli agenti permettono a TeleCode di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
TeleCode cerca in queste posizioni:

- Config di progetto: `.telecode/skills/<name>/SKILL.md`
- Config globale: `~/.config/telecode/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, TeleCode risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.telecode/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/telecode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.telecode/skills/git-release/SKILL.md` cosi':
...
TeleCode elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `telecode.json`:
...
**Per agenti integrati** (in `telecode.json`):
