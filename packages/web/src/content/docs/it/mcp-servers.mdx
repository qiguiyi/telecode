...
Puoi aggiungere strumenti esterni a TeleCode usando il _Model Context Protocol_, o MCP. TeleCode supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [TeleCode Config](https://telecode.ai/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Le organizzazioni possono fornire server MCP predefiniti tramite l'endpoint `.well-known/telecode`. Questi server possono essere disabilitati di default, lasciando agli utenti la possibilita' di fare opt-in su quelli necessari.
...
TeleCode gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, TeleCode:
...
Se il server richiede autenticazione, TeleCode ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `telecode mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, TeleCode memorizzera' i token in modo sicuro in `~/.local/share/telecode/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in TeleCode insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione TeleCode come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare TeleCode al tuo account Sentry.
