---
title: Plugin
description: Scrivi plugin per estendere TeleCode.
---

I plugin ti permettono di estendere TeleCode agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di TeleCode.
...
- `.telecode/plugins/` - plugin a livello progetto
- `~/.config/telecode/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/telecode/node_modules/`.
...
1. Config globale (`~/.config/telecode/telecode.json`)
2. Config di progetto (`telecode.json`)
3. Directory plugin globale (`~/.config/telecode/plugins/`)
4. Directory plugin di progetto (`.telecode/plugins/`)
...
TeleCode esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di telecode per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere telecode.
...
:::note
Se usi l'app desktop di TeleCode, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a telecode di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a telecode:
...
L'helper `tool` crea uno strumento personalizzato che telecode può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in telecode insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://telecode.ai/docs/sdk) per i dettagli.
