...
Crea `.telecode/commands/test.md`:
...
Puoi aggiungere comandi personalizzati tramite la config di TeleCode o creando file markdown nella directory `commands/`.

---

### JSON

Usa l'opzione `command` nella [config](/docs/config) di TeleCode:
...
- Globale: `~/.config/telecode/commands/`
- Per progetto: `.telecode/commands/`
...
telecode include diversi comandi integrati come `/init`, `/undo`, `/redo`, `/share`, `/help`; [scopri di piu](/docs/tui#commands).
