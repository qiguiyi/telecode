---
title: TUI
description: Using the TeleCode terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

TeleCode provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running TeleCode starts the TUI for the current directory.
...
When using the TeleCode TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to TeleCode. Allows you to select from available providers and add their API keys.
...
Exit TeleCode. _Aliases_: `/quit`, `/q`
...
This is separate from `telecode.json`, which configures server/runtime behavior.
...
- `leader_timeout` - Controls how long TeleCode waits after the leader key. Defaults to `2000`.
...
Use `TELECODE_TUI_CONFIG` to load a custom TUI config path.
...
- `sound_pack` - Sound pack ID to use. Defaults to `telecode.default`.
