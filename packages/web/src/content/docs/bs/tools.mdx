...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. TeleCode dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u TeleCode.
...
:::note
Ovaj alat je dostupan samo kada je `TELECODE_EXPERIMENTAL_LSP_TOOL=true` (ili `TELECODE_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz TeleCode provajdera ili kada je varijabla `TELECODE_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju TeleCode:
