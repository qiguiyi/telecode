---
title: Konfiguracija
description: Korištenje TeleCode JSON konfiguracije.
---

Možete konfigurirati TeleCode koristeći JSON konfiguracijski fajl.

---

## Format

TeleCode podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
1. **Udaljena konfiguracija** (od `.well-known/telecode`) - organizacijske postavke
2. **Globalna konfiguracija** (`~/.config/telecode/telecode.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`TELECODE_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`telecode.json` u projektu) - postavke specifične za projekat
5. **`.telecode` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`TELECODE_CONFIG_CONTENT` env var) - runtime preinačenja
...
:::note
Direktoriji `.telecode` i `~/.config/telecode` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Organizacije mogu pružiti zadanu konfiguraciju preko `.well-known/telecode` krajnje tačke. Ovo se automatski preuzima kada se autentifikujete kod provajdera koji to podržava.
...
Postavite svoju globalnu TeleCode konfiguraciju u `~/.config/telecode/telecode.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.

Za postavke specifične za TUI, koristite `~/.config/telecode/tui.json`.
...
Dodajte `telecode.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Kada se TeleCode pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `TELECODE_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `TELECODE_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.telecode` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.telecode`, tako da **može nadjačati** njihove postavke.

---

## Šema

Konfiguracijski fajl ima šemu koja je definirana u [**`telecode.ai/config.json`**](https://telecode.ai/config.json).

TUI konfiguracija koristi [**`telecode.ai/tui.json`**](https://telecode.ai/tui.json).
...
Koristite `TELECODE_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.

Stari `theme`, `keybinds`, i `tui` ključevi u `telecode.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Možete konfigurirati postavke servera za naredbe `telecode serve` i `telecode web` putem opcije `server`.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš TeleCode server.
- `mdnsDomain` - Prilagođeno ime domene za mDNS servis. Zadano je `telecode.local`. Korisno za pokretanje više instanci na istoj mreži.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj TeleCode konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, TeleCode pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/telecode/agents/` ili `.telecode/agents/`. [Saznajte više ovdje](/docs/agents).
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, TeleCode će se vratiti na `"build"` s upozorenjem.

Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`telecode run`), desktop aplikaciju i GitHub Action.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/telecode/commands/` ili `.telecode/commands/`. [Saznajte više ovdje](/docs/commands).
...
TeleCode će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, TeleCode **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju TeleCode sa prilagođenim alatima, kukicama i integracijama.

Postavite datoteke dodataka u `.telecode/plugins/` ili `~/.config/telecode/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
...
Ovo je korisno kada želite da ograničite TeleCode da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
