---
title: Server
description: Komunicirajte s telecode serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `telecode serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji telecode klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `telecode.local` |
...
Postavite `TELECODE_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `telecode`, ili postavite `TELECODE_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `telecode serve` i za `telecode web`.
...
Kada pokrenete `telecode`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite telecode server za programsku interakciju sa telecode-om.
:::

Ova arhitektura omogucava telecode podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `telecode serve` da startate standalone server. Ako je telecode TUI vec pokrenut, `telecode serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste TeleCode [IDE](/docs/ide) pluginovi.
...
telecode server izlaže sljedece API-je.
