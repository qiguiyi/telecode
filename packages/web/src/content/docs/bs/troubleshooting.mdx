...
Da biste otklonili probleme s TeleCode, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\telecode\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `telecode --log-level DEBUG`.

---

## Pohrana

TeleCode pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/telecode/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\telecode`
...
TeleCode Desktop pokreće lokalni TeleCode server (`telecode-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `TeleCode` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/telecode/telecode.jsonc` (ili `~/.config/telecode/telecode.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/telecode/telecode.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\telecode\telecode.jsonc`
...
TeleCode također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/telecode/plugins/`
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\telecode\plugins`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga TeleCode mogao ponovo izgraditi.

1. Potpuno zatvorite TeleCode Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/telecode`
- **Linux**: obrišite `~/.cache/telecode` (ili pokrenite `rm -rf ~/.cache/telecode`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\telecode`

3. Ponovo pokrenite TeleCode Desktop.

---

### Rješavanje problema sa vezom na serveru

TeleCode Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `telecode.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.

#### Provjerite varijable okruženja

Ako ste postavili `TELECODE_PORT` u svom okruženju, desktop aplikacija će pokušati da koristi taj port za lokalni server.

- Poništite `TELECODE_PORT` (ili odaberite slobodan port) i ponovo pokrenite.
...
Na Windows-u, TeleCode Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa TeleCode karakteristikama.

---

### Obavještenja se ne prikazuju

TeleCode Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za TeleCode u postavkama vašeg OS-a, i
...
1. Zatvorite TeleCode Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka TeleCode Desktop aplikacije):

- `telecode.settings.dat` (URL zadanog servera za desktop)
- `telecode.global.dat` i `telecode.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
Da saznate kojim modelima imate pristup, pokrenite `telecode models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\telecode`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. TeleCode dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\telecode`

2. Ponovo pokrenite TeleCode da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti TeleCode da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
TeleCode će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
