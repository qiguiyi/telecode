...
U TeleCode mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, TeleCode koristi nasu `telecode` temu.
...
TeleCode dolazi sa vise ugradenih tema.
...
- Zele da TeleCode odgovara izgledu njihovog terminala
...
TeleCode podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/telecode/themes/*.json` ili `$XDG_CONFIG_HOME/telecode/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.telecode/themes/*.json`
4. **Trenutni radni direktorij** - `./.telecode/themes/*.json`
