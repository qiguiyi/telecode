---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u TeleCode.
---

import config from "../../../../config.mjs"
export const console = config.console

TeleCode koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj TeleCode konfiguraciji.
...
u `~/.local/share/telecode/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem TeleCode
config.
...
TeleCode Zen je lista modela koje je obezbedio TeleCode tim koji su bili
testirano i potvrđeno da dobro radi sa TeleCode. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa TeleCode Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite telecode i idite na [telecode.ai/auth](https://telecode.ai/auth).
...
Radi kao i svaki drugi provajder u TeleCode i potpuno je opcionalan za korištenje.

---

## TeleCode Go

TeleCode Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža TeleCode tim i koji su testirani i verificirani da dobro rade s TeleCode-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `TeleCode Go` i idite na [telecode.ai/auth](https://telecode.ai/zen).
...
Da biste koristili Amazon Bedrock s TeleCode:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete telecode:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `telecode.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u TeleCode.
:::
...
Možete konfigurirati telecode za korištenje lokalnih modela preko [Atomic Chata](https://atomic.chat) — desktop aplikacije koja pokreće lokalne LLM-ove iza OpenAI-kompatibilnog API servera (zadana krajnja tačka `http://127.0.0.1:1337/v1`).
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi telecode ispravno radio.
   :::
...
Također možete dodati modele kroz svoju telecode konfiguraciju.
...
da pohrani token u telecode auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
TeleCode koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate TeleCode
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`telecode.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Prilagodite putem `telecode.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s telecode:
...
Za korištenje Google Vertex AI s TeleCode:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće telecode.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem telecodea, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [telecode-helicone-session](https://github.com/H2Shami/telecode-helicone-session) da automatski prijavite svaki TeleCode razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki TeleCode razgovor naveden kao zasebna sesija.
...
Možete konfigurirati telecode za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati telecode za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati telecode za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za TeleCode. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/telecode) za detalje.
:::
...
Da biste koristili Ollama Cloud s TeleCode:
...
3. Kopirajte API ključ za korištenje u TeleCode.
...
6. **Važno**: Prije upotrebe modela oblaka u TeleCode, morate lokalno povući informacije o modelu:
...
TeleCode Zen je lista testiranih i verifikovanih modela koju je obezbedio TeleCode tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>TeleCode Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **TeleCode Zen**.

```txt
   /connect
```

3. Unesite svoj TeleCode API ključ.
...
Također možete dodati dodatne modele putem vaše telecode konfiguracije.
...
5. Također ih možete prilagoditi putem vaše telecode konfiguracije. Evo primjera navođenja provajdera
...
Da biste koristili [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) sa TeleCodeom:
...
Također možete prilagoditi modele kroz svoju telecode konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s telecode-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `telecode.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju TeleCode da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `telecode auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite TeleCode konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj telecode konfiguraciji.
