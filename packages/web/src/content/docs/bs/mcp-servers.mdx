...
Možete dodati vanjske alate u TeleCode koristeći _Model Context Protocol_, ili MCP. TeleCode podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [TeleCode Config](https://telecode.ai/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/telecode`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
TeleCode automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, TeleCode će:
...
Ako server zahtijeva autentifikaciju, TeleCode će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `telecode mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, TeleCode sigurno čuva tokene u `~/.local/share/telecode/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u TeleCode, zajedno s ugrađenim alatima. Možete njima upravljati kroz TeleCode konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete TeleCode sa vašim Sentry nalogom.
