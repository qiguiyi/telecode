---
title: Dodaci
description: Napišite vlastite dodatke za proširenje TeleCode.
---

Dodaci vam omogućavaju da proširite TeleCode spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja TeleCode.
...
- `.telecode/plugins/` - Dodaci na nivou projekta
- `~/.config/telecode/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/telecode/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/telecode/telecode.json`)
2. Konfiguracija projekta (`telecode.json`)
3. Globalni direktorij dodataka (`~/.config/telecode/plugins/`)
4. Direktorij dodataka projekta (`.telecode/plugins/`)
...
TeleCode pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
- `client`: TeleCode SDK klijent za interakciju sa AI.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje TeleCode.
...
Spriječite telecode da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u telecode:
...
Pomoćnik `tool` kreira prilagođeni alat koji telecode može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za telecode zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://telecode.ai/docs/sdk) za detalje.
