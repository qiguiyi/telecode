...
Agent skills omogucavaju TeleCode da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
TeleCode pretrazuje ove lokacije:

- Konfiguracija projekta: `.telecode/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/telecode/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, TeleCode ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.telecode/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/telecode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.telecode/skills/git-release/SKILL.md` ovako:
...
TeleCode navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `telecode.json`:
...
**Za ugradene agente** (u `telecode.json`):
