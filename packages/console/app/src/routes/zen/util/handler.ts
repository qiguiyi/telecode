...
import { and, Database, eq, isNull, lt, or, sql } from "@telecode-ai/console-core/drizzle/index.js"
import { KeyTable } from "@telecode-ai/console-core/schema/key.sql.js"
import { BillingTable, LiteTable, SubscriptionTable, UsageTable } from "@telecode-ai/console-core/schema/billing.sql.js"
import { centsToMicroCents } from "@telecode-ai/console-core/util/price.js"
import { getMonthlyBounds, getWeekBounds } from "@telecode-ai/console-core/util/date.js"
import { Identifier } from "@telecode-ai/console-core/identifier.js"
import { Billing } from "@telecode-ai/console-core/billing.js"
import { Actor } from "@telecode-ai/console-core/actor.js"
import { WorkspaceTable } from "@telecode-ai/console-core/schema/workspace.sql.js"
import { ZenData } from "@telecode-ai/console-core/model.js"
...
import { ProviderTable } from "@telecode-ai/console-core/schema/provider.sql.js"
...
import { Resource } from "@telecode-ai/console-resource"
...
export async function handler(
  ...
  try {
    ...
    const sessionId = input.request.headers.get("x-telecode-session") ?? ""
    const requestId = input.request.headers.get("x-telecode-request") ?? ""
    const ocClient = input.request.headers.get("x-telecode-client") ?? ""
    const projectId = input.request.headers.get("x-telecode-project") ?? ""
    ...
    const retriableRequest = async (retry: RetryOptions = { excludeProviders: [], retryCount: 0 }) => {
      ...
      const res = await fetchWith429Retry(reqUrl, {
        ...
        headers: (() => {
          ...
          headers.delete("x-telecode-request")
          headers.delete("x-telecode-session")
          headers.delete("x-telecode-project")
          headers.delete("x-telecode-client")
          return headers
        })(),
        body: reqBody,
      })
  ...
  function validateBilling(authInfo: AuthInfo, modelInfo: ModelInfo): BillingSource {
    ...
    if (opts.modelList === "lite" && authInfo.billing.lite && authInfo.lite) {
      try {
        const consoleGoUrl = `https://telecode.ai/workspace/${authInfo.workspaceID}/go`
    ...
    const billingUrl = `https://telecode.ai/workspace/${authInfo.workspaceID}/billing`
    const membersUrl = `https://telecode.ai/workspace/${authInfo.workspaceID}/members`
