...
describe("Npm.sanitize", () => {
  test("keeps normal scoped package specs unchanged", () => {
    expect(Npm.sanitize("@telecode/acme")).toBe("@telecode/acme")
    expect(Npm.sanitize("@telecode/acme@1.0.0")).toBe("@telecode/acme@1.0.0")
    expect(Npm.sanitize("prettier")).toBe("prettier")
  })

  test("handles git https specs", () => {
    const spec = "acme@git+https://github.com/telecode/acme.git"
    const expected = win ? "acme@git+https_//github.com/telecode/acme.git" : spec
