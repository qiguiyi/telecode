...
import { GoogleVertexPlugin } from "@telecode-ai/core/plugin/provider/google-vertex"
      ...
      yield* catalog.transform((catalog) =>
        catalog.provider.update(ProviderV2.ID.telecode, (provider) => {
          provider.api = {
            ...
            url: "https://telecode.ai/zen/v1",
          }
        }),
      )
      yield* addPlugin()

      const provider = required(yield* catalog.provider.get(ProviderV2.ID.telecode))
