...
import { Watcher } from "@telecode-ai/core/filesystem/watcher"
...
const flagsLayer = ConfigProvider.layer(
  ConfigProvider.fromUnknown({
    TELECODE_EXPERIMENTAL_FILEWATCHER: "true",
    TELECODE_EXPERIMENTAL_DISABLE_FILEWATCHER: "false",
  }),
)
...
function withTmp<A, E, R>(
  ...
  return Effect.acquireRelease(
    Effect.promise(async () => {
      ...
      await $`git config user.email test@telecode.test`.cwd(tmp.path).quiet()
