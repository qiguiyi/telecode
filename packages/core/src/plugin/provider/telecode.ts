...
import type { CredentialValue } from "@telecode-ai/sdk/v2/types"
...
const defaultServer = "https://console.telecode.ai"
const clientID = "telecode-cli"
...
export const TeleCodePlugin = define<HttpClient.HttpClient | EventV2.Service | Scope.Scope>({
  id: "telecode",
  effect: Effect.fn(function* (ctx) {
    ...
    const load = Effect.fn("TeleCodePlugin.load")(function* () {
      const connection = yield* ctx.integration.connection.active("telecode")
      ...
      providers = credential
        ? yield* fetchProviders(http, credential).pipe(
            Effect.catch((cause) =>
              Effect.logWarning("failed to load TeleCode provider config", { cause }).pipe(Effect.as(undefined)),
            ),
          )
        : undefined
    })
    ...
    yield* ctx.integration.transform((draft) => {
      draft.update("telecode", (integration) => {
        integration.name = "TeleCode"
      })
      draft.method.update(oauth(http))
      draft.method.update({ integrationID: "telecode", method: { type: "key", label: "API key (service account)" } })
    })
    ...
    yield* ctx.catalog.transform((catalog) => {
      for (const [providerID, item] of Object.entries(providers ?? {})) {
        catalog.provider.update(providerID, (provider) => {
          provider.integrationID = Integration.ID.make("telecode")
      ...
      const item = catalog.provider.get(ProviderV2.ID.telecode)
      if (!item) return
      const hasKey = Boolean(process.env.TELECODE_API_KEY || connected || item.provider.request.body.apiKey)
    ...
    yield* events.subscribe(Integration.Event.ConnectionUpdated).pipe(
      Stream.filter((event) => event.data.integrationID === Integration.ID.make("telecode")),
