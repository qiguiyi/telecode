...
export function fileLogger(file = path.join(Global.Path.log, "telecode.log"), id: string = runID) {
...
export function minimumLogLevel() {
  const value = process.env.TELECODE_LOG_LEVEL?.toUpperCase()
  const levels = {
    DEBUG: "Debug",
    INFO: "Info",
    WARN: "Warn",
...
export function loggers() {
  return process.env.TELECODE_PRINT_LOGS === "1" ? [fileLogger(), stderrLogger] : [fileLogger()]
}
