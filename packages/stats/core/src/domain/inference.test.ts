...
describe("inference stat normalization", () => {
  ...
  test("uses provider.model to resolve telecode route providers", () => {
    ...
    expect(statProvider("big-pickle", "claude-sonnet-4-5", "telecode")).toBe("anthropic")
    expect(statProvider("big-pickle", "gpt-5", "telecode")).toBe("openai")
    expect(statProvider("big-pickle", "", "telecode")).toBe("unknown")
  ...
  test("model aggregates prefer provider.model and use normalized model", () => {
    ...
    expect(
      toModelAggregate({ ...aggregate("big-pickle", "telecode"), provider_model: "claude-sonnet-4-5" }),
    ).toMatchObject([
      {
        provider: "anthropic",
  ...
  test("provider aggregates never keep telecode as the provider", () => {
    expect(toProviderAggregate({ ...aggregate("big-pickle", "telecode"), provider_model: "gpt-5" })).toMatchObject([
      { provider: "openai" },
    ])
    expect(toProviderAggregate(aggregate("big-pickle", "telecode"))).toMatchObject([{ provider: "unknown" }])
  })

  test("geo aggregates never keep telecode or big-pickle dimensions", () => {
    expect(toGeoAggregate({ ...aggregate("big-pickle", "telecode"), country: "US" })).toMatchObject([
