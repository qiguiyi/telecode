...
const statsHomeTitle = "AI Model Usage Rankings | TeleCode Data"
const statsHomeDescription =
  "Explore TeleCode Go usage across AI models, including token volume, rankings, market share, token pricing, session cost, cache ratio, and geo breakdowns."
const statsHomeFallbackUrl = "https://telecode.ai/data/"
...
function getStatsHomeUrl(base: string, requestUrl: string) {
  ...
  if (url.hostname === "stats.telecode.ai") return "https://telecode.ai/data/"
  if (url.hostname === "stats.dev.telecode.ai") return "https://dev.telecode.ai/data/"
  return url.toString()
}
  ...
  return (
    <section id="top-models" data-section="top-models">
      <h2 data-slot="top-models-title">
        <strong>Top models.</strong> <span>Usage of models across TeleCode Go.</span>
      </h2>
      <Show
...
function UniqueUsersSection(props: { data: StatsHomeData["users"] }) {
  ...
  return (
    <section id="unique-users" data-section="unique-users">
      ...
      <SectionTitle title="Unique Users" description="Daily unique TeleCode Go users by model." />
      <Show
        when={data().some((item) => usageTotal(item) > 0)}
        fallback={
