import { Message, Model, Part, Session, SessionStatus, SnapshotFileDiff, UserMessage } from "@telecode-ai/sdk/v2"
import { SessionTurn } from "@telecode-ai/session-ui/session-turn"
import { SessionReview } from "@telecode-ai/session-ui/session-review"
import { DataProvider } from "@telecode-ai/session-ui/context"
import { FileComponentProvider } from "@telecode-ai/ui/context/file"
import { WorkerPoolProvider } from "@telecode-ai/ui/context/worker-pool"
import { createAsync, query, useParams } from "@solidjs/router"
import { createMemo, createSignal, ErrorBoundary, For, Match, Show, Switch } from "solid-js"
import { Share } from "~/core/share"
import { Logo, Mark } from "@telecode-ai/ui/logo"
import { IconButton } from "@telecode-ai/ui/icon-button"
import { ProviderIcon } from "@telecode-ai/ui/provider-icon"
import { iife } from "@telecode-ai/core/util/iife"
import { Binary } from "@telecode-ai/core/util/binary"
import { NamedError } from "@telecode-ai/core/util/error"
import { DateTime } from "luxon"
import { createStore } from "solid-js/store"
import NotFound from "../[...404]"
import { Tabs } from "@telecode-ai/ui/tabs"
import { MessageNav } from "@telecode-ai/session-ui/message-nav"
import { FileSSR } from "@telecode-ai/session-ui/file-ssr"
...
const ClientOnlyWorkerPoolProvider = clientOnly(() =>
  import("@telecode-ai/session-ui/pierre/worker").then((m) => ({
      ...
      <Show when={data()}>
        {(data) => {
          ...
          const ogImage = createMemo(() => {
            ...
            return `https://social-cards.sst.dev/telecode-share/${encodedTitle}.png?model=${modelParam}&version=${version}&id=${data().shareID}`
          })

          return (
            <>
              <Show when={info().title}>
                <Title>{info().title} | TeleCode</Title>
              </Show>
              <Meta name="description" content="telecode - The AI coding agent built for the terminal." />
                        ...
                        <div class="relative bg-background-stronger w-screen h-screen overflow-hidden flex flex-col">
                          <header class="h-12 px-6 py-2 flex items-center justify-between self-stretch bg-background-base border-b border-border-weak-base">
                            ...
                            <div class="flex gap-3 items-center">
                              <IconButton
                                as={"a"}
                                href="https://github.com/anomalyco/telecode"
                                target="_blank"
                                icon="github"
                                variant="ghost"
                              />
                              <IconButton
                                as={"a"}
                                href="https://telecode.ai/discord"
                                target="_blank"
                                icon="discord"
                                variant="ghost"
                              />
