# TeleCode V2 Promise Plugin API

The Promise plugin API is the async/await equivalent of `@telecode-ai/plugin/v2/effect`. It grants plugins the same two in-process capabilities:

- `hook` installs behavior at an TeleCode extension point.
...
```ts
import { define } from "@telecode-ai/plugin/v2/promise"
