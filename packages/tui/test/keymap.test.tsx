...
import { getTeleCodeModeStack, TELECODE_BASE_MODE, TeleCodeKeymapProvider, registerTeleCodeKeymap } from "../src/keymap"
...
test("legacy page key aliases compile as page keys", async () => {
  ...
  function Harness() {
    ...
    const offKeymap = registerTeleCodeKeymap(keymap, renderer, config)
...
test("mode-less bindings stay active when telecode mode changes", async () => {
  ...
  function Harness() {
    ...
    const popQuestion = getTeleCodeModeStack(keymap).push("question")
    counts.question = activeCounts()
    popQuestion()
    const popAutocomplete = getTeleCodeModeStack(keymap).push("autocomplete")
