import type { TuiDialogSelectOption, TuiPluginApi, TuiSlotProps } from "@telecode-ai/plugin/tui"
...
import type { useTeleCodeKeymap } from "../keymap"
...
type Input = {
  ...
  keymap: ReturnType<typeof useTeleCodeKeymap>
...
export function createTuiApiAdapters(input: Input): Omit<TuiPluginApi, "lifecycle"> {
  return {
    ...
    mode: {
      current() {
        return Keymap.getTeleCodeModeStack(input.keymap).current()
      },
      push(mode) {
        return Keymap.getTeleCodeModeStack(input.keymap).push(mode)
      },
    },
    route: {
      register(list) {
