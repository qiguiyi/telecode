...
export const TELECODE_BASE_MODE = "base"
export const COMMAND_PALETTE_COMMAND = "command.palette.show"

const TELECODE_MODE_KEY = "telecode.mode"

export const TeleCodeKeymapProvider = KeymapProvider
export const useTeleCodeKeymap = useKeymap

export { useBindings, useKeymapSelector }

export type OpenTuiKeymap = ReturnType<typeof useKeymap>
type TeleCodeModeStack = ReturnType<typeof createTeleCodeModeStack>
...
const modeStacks = new WeakMap<OpenTuiKeymap, TeleCodeModeStack>()

function isVisiblePaletteCommand(command: Command) {
  return command.hidden !== true && command.name !== COMMAND_PALETTE_COMMAND
}

export function createTeleCodeModeStack(keymap: OpenTuiKeymap) {
  keymap.setData(TELECODE_MODE_KEY, TELECODE_BASE_MODE)

  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(TELECODE_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(TELECODE_MODE_KEY, stack.at(-1)?.mode ?? TELECODE_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? TELECODE_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(TELECODE_MODE_KEY, undefined)
...
export function useTeleCodeModeStack() {
  return getTeleCodeModeStack(useTeleCodeKeymap())
}

export function getTeleCodeModeStack(keymap: OpenTuiKeymap) {
  const value = modeStacks.get(keymap)
  if (!value) throw new Error("TeleCode mode stack is not registered for this keymap")
  return value
}
...
export function registerTeleCodeKeymap(keymap: OpenTuiKeymap, renderer: CliRenderer, config: ResolvedKeymapConfig) {
  const modeStack = createTeleCodeModeStack(keymap)
...
export function useCommandSlashes(): Accessor<readonly CommandSlashEntry[]> {
  const keymap = useTeleCodeKeymap()
