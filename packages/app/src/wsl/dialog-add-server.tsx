...
import { showToast } from "@telecode-ai/ui/toast"
import { createEffect, createMemo, For, Match, onCleanup, Show, Switch } from "solid-js"
import { createStore } from "solid-js/store"
import { useLanguage } from "@/context/language"
import { usePlatform } from "@/context/platform"
import { useWslServers } from "./context"
import { enterWslTeleCodeStep } from "./settings-model"

type WslServerStep = "wsl" | "distro" | "telecode"

const STEPS: WslServerStep[] = ["wsl", "distro", "telecode"]
...
export function DialogAddWslServer(props: DialogWslServerProps = {}) {
  ...
  const telecodeCheck = createMemo(() => {
    const distro = selectedDistro()
    if (!distro) return null
    return current()?.telecodeChecks[distro] ?? null
  })
  ...
  const telecodeReady = createMemo(() => {
    const check = telecodeCheck()
  ...
  const installingTeleCode = createMemo(() => {
    const job = current()?.job
    return job?.kind === "install-telecode" && job.distro === selectedDistro()
  })
  const allReady = createMemo(() => wslReady() && distroReady() && telecodeReady())
  const addDisabled = createMemo(() => {
    const job = current()?.job
    if (!job) return store.adding
    return store.adding || job.kind !== "probe-telecode"
  })
  ...
  const autoProbe = createMemo(() => {
    ...
    if (!state.telecodeChecks[distro]) {
      return { key: `probe-telecode:${distro}`, run: () => api.probeTeleCode(distro) }
    }
    return null
  })
  ...
  const telecodeMessage = createMemo(() => {
    const state = current()
    if (!state) return language.t("wsl.onboarding.checkingTeleCode")
    const distro = selectedDistro()
    if (state.job?.kind === "install-telecode") {
      return distro
        ? language.t("wsl.onboarding.updatingTeleCodeIn", { distro })
        : language.t("wsl.onboarding.updatingTeleCode")
    }
    if (state.job?.kind === "probe-telecode") {
      return distro
        ? language.t("wsl.onboarding.checkingTeleCodeIn", { distro })
        : language.t("wsl.onboarding.checkingTeleCode")
    }
    if (telecodeCheck()?.error) return telecodeCheck()!.error
    if (telecodeCheck()?.matchesDesktop === false) {
      return distro
        ? language.t("wsl.onboarding.updateTeleCodeIn", { distro })
        : language.t("wsl.onboarding.updateTeleCode")
    }
    if (telecodeReady()) {
      return distro
        ? language.t("wsl.onboarding.telecodeReadyIn", { distro })
        : language.t("wsl.onboarding.telecodeReady")
    }
    return distro
      ? language.t("wsl.onboarding.installTeleCodeIn", { distro })
  ...
  const openTeleCodeStep = () => {
    const distro = selectedDistro()
    if (!distro) return
    void run(() => enterWslTeleCodeStep(distro, api.probeTeleCode, (step) => setStore("step", step)))
  }
        ...
        title:
          step === "wsl"
            ...
            : step === "distro"
              ...
              : language.t("wsl.onboarding.step.telecode"),
        state:
          active === step
            ? "current"
            : step === "wsl"
              ...
              : step === "distro"
                ? distroReady()
                  ...
                  : index > activeIndex
                    ? "locked"
                    : "warning"
                : telecodeCheck()?.matchesDesktop === false
                  ? "warning"
                  : telecodeReady()
                    ? "done"
                    : index > activeIndex
      ...
      >
        ...
        >
          ...
          <Switch>
            ...
            <Match when={activeStep() === "telecode"}>
              <div class="rounded-md bg-surface-base p-4 flex flex-col gap-3">
                <div class="flex items-center justify-between gap-3">
                  <div class="text-14-medium text-text-strong">{language.t("wsl.onboarding.step.telecode")}</div>
                  <div class="flex items-center gap-2">
                    <Show when={selectedDistro()}>
                      <Button
                        variant="ghost"
                        size="large"
                        disabled={busy()}
                        onClick={() => runSelectedDistro((distro) => api.probeTeleCode(distro))}
                      >
                        {language.t("wsl.onboarding.refresh")}
                      </Button>
                    </Show>
                    <Show when={!telecodeReady() || telecodeCheck()?.matchesDesktop === false}>
                      <Button
                        variant="secondary"
                        size="large"
                        disabled={busy()}
                        onClick={() => runSelectedDistro((distro) => api.installTeleCode(distro))}
                      >
                        <Show when={installingTeleCode()}>
                          <Spinner class="size-4 shrink-0" />
                        </Show>
                        {telecodeCheck()?.resolvedPath
                          ? language.t("wsl.onboarding.updateTeleCode")
                          : language.t("wsl.onboarding.installTeleCode")}
                      </Button>
                    </Show>
                  </div>
                </div>
                <div class="text-12-regular text-text-weak whitespace-pre-wrap break-words">{telecodeMessage()}</div>
                <Show when={telecodeCheck()?.matchesDesktop === false ? telecodeCheck() : null}>
                  {(check) => (
                    <div class="rounded-md border border-border-weak-base px-3 py-3 flex flex-col gap-1">
                      <div class="text-12-regular text-text-weak">
          ...
          <Show when={activeStep() === "telecode" && allReady() && selectedDistro()}>
