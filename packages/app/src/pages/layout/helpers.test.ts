...
import { type Session } from "@telecode-ai/sdk/v2/client"
import {
  childSessionOnPath,
  closeHomeProject,
  displayName,
...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("telecode://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("telecode://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("telecode://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("telecode://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("telecode://open-project")).toBeUndefined()
    expect(parseDeepLink("telecode://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "telecode://open-project?directory=/a",
      "telecode://other?directory=/b",
      "telecode://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("telecode://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("telecode://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("telecode://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("telecode://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "telecode://new-session?directory=/a",
      "telecode://open-project?directory=/b",
      "telecode://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __TELECODE__: {
        deepLinks: ["telecode://open-project?directory=/a"],
      },
    } as unknown as Window & { __TELECODE__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["telecode://open-project?directory=/a"])
