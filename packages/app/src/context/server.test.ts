  ...
  test("moves an existing canonical web bucket into local scope", () => {
    expect(
      migrateCanonicalLocalServerState(
        {
          list: [],
          projects: { "https://telecode.example.com": [{ worktree: "/remote", expanded: true }] },
          lastProject: { "https://telecode.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://telecode.example.com"),
      ),
    ).toEqual({
      list: [],
      ...
      migrateCanonicalLocalServerState(
        {
          projects: {
            ...
            "https://telecode.example.com": [
              { worktree: "/local", expanded: true },
              { worktree: "/remote", expanded: true },
            ],
          },
          lastProject: { local: "/local", "https://telecode.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://telecode.example.com"),
      ),
    ).toEqual({
