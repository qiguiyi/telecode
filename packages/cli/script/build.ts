...
for (const item of targets) {
  ...
  const result = await Bun.build({
    ...
    define: {
      ...
      TELECODE_CLI_NAME: `'${binary}'`,
      TELECODE_MODELS_DEV: modelsData,
      TELECODE_CHANNEL: `'${Script.channel}'`,
      TELECODE_LIBC: item.os === "linux" ? `'${item.abi ?? "glibc"}'` : "undefined",
  ...
  await Bun.write(
    ...
    JSON.stringify(
      {
        name: `@telecode-ai/${name}`,
        version: Script.version,
        license: "MIT",
        repository: { type: "git", url: "git+https://github.com/anomalyco/telecode.git" },
        os: [item.os],
        cpu: [item.arch],
      },
      null,
