...
function ref(request: HttpServerRequest.HttpServerRequest): Location.Ref {
  ...
  const workspaceID = query.get("location[workspace]") || request.headers["x-telecode-workspace"]
  const directory =
    query.get("location[directory]") ||
    (request.headers["x-telecode-directory"] ? decode(request.headers["x-telecode-directory"]) : process.cwd())
